...
最后更新：2026-08-03
维护者：主 Agent
状态：`lxc/workbench-v4` 已部署 R0-12 正向视觉导演提示词修复；普通员工 Prompt 框只显示后端中文终稿，不再把内部英文生图指令改写成“画面/主体/动作/构图”字段摘要，N5/N6 模板要求最终可见提示词聚焦构图、灯光、材质、画面内容和版式文字。Prompt OS 4.1 的完整营销质量、真实 ERP/OSS、新付费 1+8、50 张压力测试和全量历史测试宽门禁收口仍未完成。
