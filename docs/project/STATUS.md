# 项目控制板

最后更新：2026-07-30
维护者：主 Agent
状态：Prompt OS、审核/导出后端、白底主图顺序门禁、ERP 登录切片与 OSS 对象存储已部署到 2 号服务器；OSS smoke 已通过，ERP 与 APIMart 真实 smoke 被服务器出站网络阻断，动态公平调度与 HTTPS 发布仍未启用

## 当前目标

把技术验证 MVP 重设为公司内部的批量 AI 商品图生产平台。体验主线：工作台 → 项目 → 上传与 SKU 分组 → 商品创作台 → 批量生产 → 审核与导出。

## 角色会话

| 角色 | 会话职责 | 当前交付 |
| --- | --- | --- |
| 主 Agent | 优先级、集成、风险与发布签核 | 本控制板、交付规格、任务拆分 |
| 产品与 Prompt OS | 商品事实、身份锁、Brief、套图意图、验收与变更控制 | Prompt OS 领域契约 |
| 前端体验 | React 工作台、信息架构、交互和可见状态 | 工作台与创作台体验 |
| 后端平台 | API、数据、权限、预检、任务状态、额度与存储 | 可被前端消费的受限接口 |
| 平台规则/合规 | 官方规则、模板、审核 checklist、来源与版本 | 可发布的规则/模板输入 |
| QA/发布 | 自动回归、手工验收、安全门禁、预览与发布报告 | 假模式验收和发布证据 |

## 当前任务契约

| ID | Owner | 目标 | 输入契约 | 输出契约 | 文件边界 | 验证 | 状态 | 阻塞 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| C0-01 | 主 Agent | 固定运营术语与体验路线 | 业务确认 | 统一的“项目、商品/SKU、输出图、版本、Brief”术语 | 协作/产品文档 | 文档交叉核对 | 已完成 | 无 |
| C0-02 | 主 Agent | 盘点前端、后端、部署基线 | 现有仓库与运行手册 | 可复用能力和生产缺口 | 盘点/运行文档 | 只读核查 | 已完成 | 无 |
| C0-03 | 主 Agent | 建立主 Agent 项目治理 | 产品方向 | 角色、交接和签核规则 | `CLAUDE.md`、本控制板 | 规则与软链核对 | 已完成 | 无 |
| C0-04 | 主 Agent | 确认前端技术栈 | 交互/性能目标 | React + TypeScript + Vite、TanStack Query、dnd-kit 与同源 Django API | 前端架构文档 | 架构审阅 | 已完成 | 无 |
| P0-01 | 产品与 Prompt OS | 结构化 Prompt、事实与身份锁 | 产品方案、已发布模板 | 模型/编译器契约；竞品只形成 Style DNA，不作生成参考 | 模型、服务、迁移、测试 | Prompt OS 与全量后端回归 | 已完成 | 规则和模板按站点持续发布 |
| P0-02 | 前端体验 | 商品生产工作台重构 | 已有 API、已确认体验规格、SKU 导入需求和产品方向 A | 品牌化登录、SKU 粘贴导入、引导创建、商品确认板、商品创作台、生产/审核/导出工作台 | `frontend/**`、认证模板、体验规格 | 视觉验收、组件/API 合同测试、生产构建 | 进行中 | 体验方向 A、创作台、审核闭环与选择式本地 ZIP 导出已确认；等待规格审阅后开始集群 1 实施，真实 HTTPS 会话烟测待发布环境 |
| P0-03 | 后端平台 | 审核、受限媒体、导出与 SKU 商品资料导入 API | Prompt OS 契约与商品资料接口响应 | 权限受控审核/导出/快照接口；ERP 登录影子用户、当前用户 Token 查询 SKU、私有 OSS 归档；同模板白底第 1 槽完成后才提交第 2–8 槽，失败/取消仅重做主图 | Django 应用、迁移、测试、Compose | API/权限/全量回归、服务器 health/OSS smoke | 进行中 | 本切片已部署到 2 号服务器；容器已拿到 ERP/OSS/APIMart 环境键，OSS 写读删通过。ERP 登录和 SKU 查询因服务器到 `103.198.125.5:16777/8077` 超时未通过，需放行服务器出口 IP `139.224.2.166` 或提供可达网络路径 |
| P0-04 | 平台规则/合规 | 独立 `template-seed` 规则/模板种子 | 官方来源、站点范围和管理员发布标准 | 8 张、1:1、1k 默认套图；Shopee/TikTok 种子与来源登记册；TikTok 主图和带文字辅助图分槽审核 | 规则文档、种子数据 | 来源/版本人工复核 | 进行中 | 正在按站点整理官方规则，发布仅限管理员刘学城 |
| P0-05 | QA/发布 | 假模式发布门禁与运维收口 | Compose、Caddy、现有测试 | 同源静态部署、运行手册、验证证据 | `docker/**`、Compose、环境模板、运行文档 | Compose 静态解析、服务器构建、health/OSS smoke | 进行中 | 2 号服务器 Docker Compose 构建和 `/health/ready` 通过；HTTP 临时入口仍非正式 HTTPS 发布 |
| P0-06 | 主 Agent + 平台/QA | APIMart 真实契约与费用门禁 | APIMart 中文文档、公开测试图、精确目标模型 | 三模型的真实包络适配、结构化输出、异步任务恢复与质量基准证据 | 视觉/Prompt/图像适配器、测试、运行文档 | 公开图契约、标注基准集、失败语义、任务轮询与费用记录 | 进行中 | APIMart Base URL 已按官方 `https://api.apimart.ai/v1` 兼容并写入服务器 Secret；服务器到 `api.apimart.ai:443` 连接超时，真实 smoke 暂未通过。需修复 2 号服务器到 APIMart 的出站网络后再做付费契约 |

## 当前决定

| 日期 | 决定 | 原因 | 影响 |
| --- | --- | --- | --- |
| 2026-07-29 | 批量生产为主，单商品创作台为辅 | 适配内部运营和每日大量生成 | 不做首版自由画布 |
| 2026-07-29 | 一 Agent 一会话，主 Agent 统一集成 | 保留角色上下文并防止决策分散 | 以本控制板为唯一事实来源 |
| 2026-07-29 | 运营前端采用 React + TypeScript + Vite | 创作台、拖拽、队列和审核需要可组合状态与高质量交互 | Django 保留为同源 API、Session、任务与后台；不引入 Next.js 或额外后端基础设施 |
| 2026-07-29 | 竞品图只用于抽象策略，不可作为生成参考 | 防止商品身份、版权与合规风险 | 原图仅可到批准的 `gpt-5-nano-2025-08-07` 视觉观察器；Prompt OS、前端生成请求和 `gpt-image-2` 均不得传递竞品图 |
| 2026-07-29 | 只有人工通过的版本可导出 | 区分技术完成和业务验收 | 导出 API、审核 UI 与 ZIP 规则需共同执行 |
| 2026-07-29 | APIMart API 活跃异步任务硬上限为 500 | 支撑目标吞吐，但不将供应商 API 上限误作当前服务器能力 | 运行默认仍为 2；按 2、5、8、50、100、250、500 分级压测，完成原子认领与限流后才能提高 |
| 2026-07-29 | 正式入口只要求账号登录，不使用来源 IP 白名单 | 内部员工从公网访问 | 仍必须使用域名 HTTPS 与账号级失败锁定；HTTP IP:端口只可做技术预览 |
| 2026-07-30 | ERP 登录成功的全部用户可进入平台，刘学城是唯一初始管理员 | 运营希望沿用 ERP 权限校验和个人账号查询 | 平台创建本地影子用户；管理员身份由 `PLATFORM_ADMIN_ERP_USERS` 中的 ERP 登录名决定 |
| 2026-07-29 | 默认 8 张、1:1、1k，用户可从已发布允许项中调整 | 满足跨境商品页的首轮生产需求 | 规则模板按平台/市场决定槽位；TikTok 主图不叠加促销文字 |
| 2026-07-29 | 所有平台套图第 1 张固定为标准白底产品主图，并先完成再提交后续槽位 | 保证商品身份、上架基础质量与跨平台一致性 | 第 1 槽完整、真实、无促销文字/水印；同批次、同商品、同模板的后续 7 槽等待其完成，主图失败/取消时继续等待重做且不得调用供应商 |
| 2026-07-29 | 文字叠加按站点默认语言生成并人工审核 | 同时满足本地化和平台差异 | 主图与辅助图分槽；文本只能来自已证实商品事实 |
| 2026-07-29 | 闲时允许用户临时借用并发 | 在全局上限内提高资源利用率 | 基础每人 2 个活跃任务；有其他用户排队时恢复公平轮转 |
| 2026-07-29 | 竞品版式结构可作为 Style DNA 借鉴 | 满足同等商业视觉策略需求 | 可复用画面层级、版式、色彩、光线和场景密度；竞品原图仅发送给 `gpt-5-nano-2025-08-07` 观察器，不得传给生成、文本、Prompt 或导出链路，也不得复制商标、包装、人物、原文案及逐像素页面 |
| 2026-07-29 | 前端采用 A：商品生产工作台 | 当前功能页缺乏商品视觉、创作重心与品牌化登录首印象 | 新用户默认进入 SKU 导入创建，文件夹上传为补充；有进行中项目的用户默认进入生产总览。采用商品确认板、三栏创作台、人工审核闭环与选择式本地 ZIP 导出；不引入大型 UI 套件 |
| 2026-07-29 | 导出按通过版本、商品名称与 SKU 交付 | 员工需要可选择范围并放入自己的本地文件夹 | 只导出审核通过的最新版本；支持全选/筛选/选中商品/单商品/平台套图；ZIP 按“商品名称__SKU/01–08 槽位”组织，由浏览器保存窗口选择本地目录 |
| 2026-07-29 | 素材合并不等同于多角度 | 同款多色、花型、尺寸或款式是独立子 SKU，不可被自动合并 | 用户显式标记“同一可售 SKU 的补充参考”才合并；独立子 SKU 可归入商品家族并一键同时出图，但各自产出、审核和重做 |
| 2026-07-29 | SKU 是一等导入入口，图片上传保留为补充来源 | 运营已有商品资料系统，输入 SKU 即可取得商品名与产品图 | 服务端只消费输入 SKU、响应中的 `productName` 与 `pic`；SKU 保留为审计/重导入键，销售、库存、成本等字段不进入平台或 Prompt |
| 2026-07-29 | TXT 与 SKU 导入的文本框都是“种子风格提示词” | 普通运营不应编写完整生产 Prompt | 有内容时作为项目级风格偏好；为空时由营销图 Prompt 节点生成风格方案，仍不得虚构商品事实 |
| 2026-07-30 | 商品资料服务改为 ERP 登录用户自己的 Token 查询 | 运营希望登录和后续查询都按 ERP 账号权限校验 | 登录时调用 `ERP_LOGIN_URL`；所有 ERP 登录成功用户可进入平台，平台只创建影子用户绑定项目归属。SKU 查询使用服务端 session 中的当前用户 Token，仅传 `skuList`，只消费 SKU、`productName`、`pic` |
| 2026-07-29 | Prompt OS 固定使用 APIMart 的 DeepSeek V4 Pro、GPT-5 Nano 视觉观察器与 GPT Image 2 | 让自动视觉理解可用，同时隔离竞品素材与生成链路 | `gpt-5-nano-2025-08-07` 只将我方源图、竞品图或生成候选图变为 Schema 校验的视觉事实包；`deepseek-v4-pro` 处理所有文本语义节点和结构化 Prompt；`gpt-image-2` 只生成/修订图片。竞品原图只到视觉观察器，绝不进入 DeepSeek、GPT Image 2、生产 Prompt、生成任务或导出。P0 必须以 APIMart 中文文档和账户验证三个精确模型 ID，失败时阻断而不降级 |
| 2026-07-29 | APIMart 视觉观察器首次真实契约成功，不等于质量通过 | 区分“接口可调用”与“商品视觉事实可靠” | 使用公开测试图的两次最小付费调用确认 `gpt-5-nano-2025-08-07`、`input_image` 和严格 JSON 路径可用；下一步须以带人工标注的商品基准集衡量事实准确率，未通过前不得作为自动审核结论 |
| 2026-07-29 | 首个实现切片只交付“SKU → 商品卡 → 8 槽计划 → 预检”的可演示闭环 | 先解决真实入口与确认生成前的事实完整性，避免并行改 Prompt Worker、审核和导出造成迁移冲突 | 不在首切片改真实生图提交、竞品 Style DNA、审核圈选或选择式导出；白底第 1 槽仍是模板中的硬前置。默认采用新模板版本，不改写已有项目的 4 槽快照 |
| 2026-07-29 | APIMart 的真实账号包络优先于文档示例 | 同一供应商不同模型端点的外层 JSON 已出现不一致，不能依赖单一伪造包络 | `deepseek-v4-pro` 账户实测为顶层 OpenAI Chat Completions 包络，非文档示例的 `code/data`；适配器必须按端点/模型显式解析并以真实 fixture 回归。GPT Image 2 已实测 `submitted → processing → completed`、结果 URL 与费用字段，完成后必须立即受限下载归档；`cancelled` 视为终态 |

## 阻塞与风险

| 项目 | 影响 | Owner | 处理条件 |
| --- | --- | --- | --- |
| Shopee/TikTok 各站最终规则仍在逐站核对 | 不能发布超出已核对范围的自动合规承诺 | 平台规则/合规 | 官方来源登记、版本与管理员刘学城发布完成 |
| ERP 与 APIMart 从 2 号服务器出站不可达 | 登录/SKU 导入和真实付费生图无法在服务器上完成 smoke | 后端平台 + QA/发布 + 网络/ERP 负责人 | 放行服务器出口 IP `139.224.2.166` 到 `103.198.125.5:16777/8077`，并修复到 `api.apimart.ai:443` 的出站访问 |
| 动态公平并发与 500 上限执行尚未启用 | 不能把供应商上限当成当前服务器能力 | 后端平台 + QA/发布 | 原子认领/限流、真实契约测试与分级压测 |
| HTTP 临时入口 | 不能承载 100 名员工真实素材 | QA/发布 | 域名 HTTPS 与账号安全就绪；不使用来源 IP 白名单 |
| 本机无 Docker CLI | 本地不能复现 Compose 构建 | QA/发布 | 服务器 Docker Compose 已作为当前发布验证环境；本地仍只跑单元/构建测试 |
| APIMart 账户模型目录、兼容参数或响应封装与上游模型资料不同 | 不能依据原厂资料直接开发或发布 | 后端平台 + QA/发布 | 以 APIMart 中文文档及受限账户的精确模型 ID、图片输入、输出 Schema、错误/限流契约测试为准 |

## 更新协议

每次交接只更新相关任务行，并附：owner、目标、输入/输出契约、文件边界、验证命令/结果、状态、风险和下一个 owner。直接聊天得到的接口或需求决定，必须在同一工作回合回写到本文件或相应规格后才生效。跨 Agent 不得并行修改同一文件；只有主 Agent 可以合并任务或签核发布。
