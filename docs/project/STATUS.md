...
最后更新：2026-08-03
维护者：主 Agent
状态：`lxc/workbench-v4` 本轮 R0-19 已部署到预览；商品卡 Prompt 编辑区不再说“1+8”，原始图直通时只显示 8 张生成提示词并按 01–08 编号；生产 Prompt 节点超时从 20 秒提升到 120 秒，部署后 N1–N9 生产容器真实模型 smoke 均返回非空文本；N5/N6 模型失败不再 deterministic fallback 冒充成功，失败时保存具体中文业务原因。Prompt OS 4.1 的真实付费 1+8、50 张质量压测、ERP/OSS 浏览器 smoke 和全量历史测试宽门禁收口仍未完成。
