...
最后更新：2026-07-31
维护者：主 Agent
状态：商品工作台与 Prompt OS 生成链路重构进行中。当前分支正在实现“先 N1/N2 识别、首次确认平台国家、再 N3–N7/付费生成”；尚未完成浏览器、服务器或真实付费验收。
...
| R0-01 | 主 Agent + Prompt OS + 前端体验 | 商品工作台与 Prompt OS 生成链路重构 | 已确认重构计划、既有项目/商品/API | 名称式项目、首次配置、方形商品卡、统一添加抽屉、N1–N7 当前证据门禁 | 后端/前端与五份权威文档；不改 `用户操作流程以及相关触发.md` | 实现与单元测试进行中；尚未完成浏览器、部署、ERP/OSS 或真实付费 1+8 验收 | 进行中 | Worker 门禁审查修复与前端 API 对接未完成 |
...
| 日期 | 决定 | 原因 | 影响 |
| --- | --- | --- | --- |
| 2026-07-31 | 先识别、后确认市场、再生成 | 当前链路在 N1–N7 完成前创建任务并使用通用兜底 Prompt，导致重复且沿用源图构图 | 新建项目只填名称；N1/N2 可先运行，N3–N7 与付费提交等待首次平台/国家配置；无当前 N7 合格 PromptVersion 时所有 Generation 路径拒绝 |
| 2026-07-31 | 项目默认采用继承，商品只保存显式覆盖 | 运营需要先批量配置再按少数商品改市场，且修改默认不能覆盖个别修改 | 生效顺序为商品覆盖 → 项目默认 → 全球通用；历史 Prompt/结果/审核永不改写 |
| 2026-07-31 | 工作台改为统一添加抽屉和方形商品卡 | 上传/ERP 长期占位挤压商品整理空间，列表不利于批量视觉确认 | 图片与 ERP 为同一抽屉页签；单/多图、文件夹和拖拽均可添加；卡片直接编辑名称、平台、国家与补充信息 |
