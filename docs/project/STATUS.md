...
最后更新：2026-08-03
维护者：主 Agent
状态：`lxc/workbench-v4` 本轮 R0-18 本地验证完成，待部署；Shopee VN 01 原始商品图是直通参考图，不再出现在商品卡 Prompt 编辑区，02 才是标准白底产品图；N5/N6 已拿到 DeepSeek 非空文本后，不再因解析、Schema 或归一化异常把预备生成打失败。Prompt OS 4.1 的真实付费 1+8、50 张质量压测、ERP/OSS 浏览器 smoke 和全量历史测试宽门禁收口仍未完成。
...
| R0-18 | 主 Agent + Prompt OS + 前端体验 | 原始图 Prompt 槽隐藏与 N5/N6 宽松失败边界补强 | 用户指出原始商品图不该在商品卡里占提示词框；同时真实预备生成仍出现“提示词生成失败”，要求去掉 JSON/字段/Schema 这类限制，只在模型无返回或调用失败时失败 | `PromptEditor` 不渲染 `readOnly` 槽位，Shopee VN 01 原始图直通不再展示 Prompt 输入框；VN 专用槽位中文映射补齐；N5/N6 在 DeepSeek 已返回非空文本后，解析、Schema 或归一化异常都会按 `prompt_source="deepseek"` 原文继续，不走 deterministic fallback，也不创建生产 fallback PromptVersion | `platform_app/services.py`、`tests/test_prompt_os.py`、`frontend/src/components/PromptEditor.tsx`、`frontend/src/labels.ts`、`frontend/src/__tests__/workbench-v3.test.tsx`、本控制板 | N5/N6/fallback 聚焦 8 passed；前端 workbench 19 passed；`python manage.py check` passed；`npm --prefix frontend run build` passed；`git diff --check` passed（仅 Windows 换行提示） | 本地验证完成，待部署 | 完整 `tests/test_prompt_os.py` 仍有既有 N1/N2/旧 worker 断言失败；真实付费 1+8 和 50 张质量压测待验收 |
