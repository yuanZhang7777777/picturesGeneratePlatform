...
最后更新：2026-08-02
维护者：主 Agent
状态：`lxc/workbench-v4` 已完成 Prompt OS 3.4 节点与执行设计、实施计划和 GPT-5.5 任务书，尚未实现或发布。当前生产继续使用已部署运行版本；目标 3.4.0 将把整理零 AI、预备/正式生成统一链路、员工错误脱敏、多图 `target_appearances`、Prompt 变量总线、五种营销策略、目标语言文字锁和 N7 生成门禁变成可验证契约；真实 ERP/OSS、新付费 1+8 和 50 商品浏览器验收仍未完成。
