...
最后更新：2026-07-31
维护者：主 Agent
状态：阶段 1“批量整理工作台可用化”已在 `lxc/phase1-workbench` 通过本地门禁，等待部署到预览环境。阶段 2 尚未开始。

## 当前目标

按 [`2026-07-31-phased-delivery-roadmap.md`](../superpowers/plans/2026-07-31-phased-delivery-roadmap.md) 严格串行完成阶段 0–6。当前只交付阶段 1：上传预览、防重复导入、选择、删除/归档、空商品名和紧凑商品列表；阶段 2 的 ERP 与平台/国家配置不得提前混入。
...
| 1 批量整理工作台 | 前端体验 + 后端删除/归档 | `frontend/src/**`、相关 Django 模型/API/测试；Agent 文件不重叠 | 后端 206 passed；前端 51 passed；Vite build、Django check、迁移漂移和浏览器图片/文件夹 E2E 通过 | 本地门禁通过，待部署 | 预览迁移、五服务、健康与页面 smoke |
| 2 ERP 与市场配置 | ERP/配置 + 前端市场选择 | 阶段 1 门禁通过后冻结 | 真实 ERP 登录、1 SKU、OSS、项目/商品配置 E2E | 未开始 | 阶段 1 通过 |
