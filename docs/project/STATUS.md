...
最后更新：2026-08-03
维护者：主 Agent
状态：`lxc/workbench-v4` 本轮 R0-20 已完成本地验证并准备部署；N6 营销槽位改为可配置并发调用，中文 `display_prompt` 缺失或空泛时只触发一次 DeepSeek 重写，不用英文最终生图指令冒充运营可编辑提示词；空 PromptVersion 不得落库，正式出图进度不再显示 `0/0`。Prompt OS 4.1 的真实付费 1+8、50 张质量压测、ERP/OSS 浏览器 smoke 和全量历史测试宽门禁收口仍未完成。
...
| R0-20 | 主 Agent + Prompt OS + 前端体验 + 后端平台 | N6 并发、中文提示词可见性和出图进度修复 | 用户指出提示词槽不能空、运营看的应是 DeepSeek 中文图片设计稿而非最终英文生图指令；正式出图卡片显示 `出图中 · 0/0`；预备生成速度要利用更高并发且各槽互不干扰 | N6 各营销槽位在主线程完成输入渲染后按 `PROMPT_OS_SLOT_CONCURRENCY` 并发调用 DeepSeek，默认 8；Prompt Worker 默认并发 64；中文 `display_prompt` 为空、无中文或命中空泛句时只让 DeepSeek 重写一次，仍不合格则预备失败并提示“提示词生成失败，请重试预备生成”；前端只展示中文 `displayPrompt` 或人工中文稿，不展示内部英文生图指令；商品识别信息压成中文标签和折叠证据；生成进度 total 缺失时按可生成 Prompt 数量兜底，避免 `0/0` | `platform_app/services.py`、`image_platform/settings.py`、`docker-compose.yml`、`.env.example`、`platform_app/management/commands/run_prompt_worker.py`、`frontend/src/components/ProductCard.tsx`、`frontend/src/components/PromptEditor.tsx`、相关测试、本控制板 | N6 并发/Shopee 专属节点/空文本/中文 displayPrompt 重写/非 JSON 文本聚焦 6 passed；前端 workbench+prompt 32 passed；`npm --prefix frontend run build` passed | 本地验证完成，准备部署 | 完整 `tests/test_prompt_os.py` 仍有既有 N1/N2/旧 worker 断言失败；真实付费整套 1+8 质量和 50 张质量压测待运营验收 |
