...
最后更新：2026-08-03
维护者：主 Agent
状态：`lxc/workbench-v4` 本轮 R0-20 追加修复中；Prompt Worker 商品领取改为数据库行锁/原子认领以支撑多商品并发，前端不再把已有模型 Prompt 过滤成“此槽位提示词缺失”，提示词编辑改为自动保存并删除手动保存按钮，正式生成在预备中被门禁。Prompt OS 4.1 的真实付费 1+8、50 张质量压测、ERP/OSS 浏览器 smoke 和全量历史测试宽门禁收口仍未完成。
...
| R0-14 | 主 Agent + 前端体验 | 商品卡恢复平台/国家覆盖入口 | 用户指出商品卡里站点/国家入口丢失；“名称 + 补充信息”只应约束商品描述字段，不应删除会影响 Prompt/规则的生成配置覆盖 | 商品卡恢复“平台”“国家”两个轻量下拉，默认跟随项目；保存仍走既有 `platform_override` / `market_override` 后端字段；颜色、款式、风格、身份锁仍合并进补充信息 | `frontend/src/components/ProductCard.tsx`、前端回归测试、本索引和 Prompt OS 4.1 文档 | 前端 `App`/`workbench-v3` 53 passed；`npm --prefix frontend run build` passed；远端 `.deployed_commit`、Compose 状态和 `/health/ready` 通过 | 已部署到预览；回退备份 `/opt/independent-image-platform-backups/20260803_140810-d8d24b6-pre-r014-product-card-site` | 无 |
| R0-15 | 主 Agent + Prompt OS + 前端体验 | 商品卡识别回填、结果图加载和真人/尺度槽位修复 | 用户指出空商品名未及时用识别结果填上、补充信息会出现英文、结果图加载慢，且九张图仍缺少用户/模特使用图 | 商品卡按字段保护人工编辑：空名称跟随 AI 身份回填，已编辑补充信息不被覆盖；常见混合英文识别字段转中文；结果页图片使用浏览器 lazy/async；N5/N6 对 Usage/功能/操作和 Model/scale/模特/比例/尺度槽位强制传递真人、手部、宠物或空间尺度关系到中文导演稿与英文 Image2 Prompt | `frontend/src/components/ProductCard.tsx`、`frontend/src/components/ResultGrid.tsx`、`platform_app/services.py`、`platform_app/prompt_templates_v3.py`、`platform_app/management/commands/seed_platform_templates.py`、相关测试、本控制板 | 前端 `App`/`workbench-v3` 55 passed；`npm --prefix frontend run build` passed；后端新增 Model/scale + 六类 benchmark 2 passed；`tests/test_prompt_templates_v3.py` 13 passed；`manage.py check` passed；`git diff --check` passed；完整 `tests/test_prompt_os.py` 仍有既有 N1/N2/旧 worker 断言失败 | 已部署到预览；回退备份 `/opt/independent-image-platform-backups/20260803_143359-r015-identity-model-result-fixes` | 真实付费 1+8、50 张质量压测、ERP/OSS 浏览器 smoke 和全量 Prompt OS 历史测试收口待后续任务 |
| R0-16 | 主 Agent + Prompt OS + 前端体验 + 后端平台 | 结果导出免审核、结果页 Prompt 可见和 fallback 导演稿升级 | 用户指出结果页 9/9 完成仍因未审核无法导出、需要在生产结果里看到当时提示词，并要求提示词按旧工作流字数写成真正图片设计而非“核心收益/购买任务”空话，同时给模型保留天马行空的创意空间 | 结果页把所有完成且有图的最新版本作为默认勾选导出项，不再显示“待审核通过后可导出”；后端 ZIP 只要求 completed + 有结果文件；结果侧栏显示 `generation.prompt_text`；N5 fallback 八槽改为差异化创意导演 brief；N6 fallback 展示稿包含消费者文案源、画面目标、商品呈现、人物与场景、构图与镜头、光线材质、文字版式，实际 Image2 Prompt 同步包含导演稿和正向文案锁 | `frontend/src/components/ResultGrid.tsx`、`platform_app/views.py`、`platform_app/services.py`、`platform_app/prompt_templates_v3.py`、`platform_app/management/commands/seed_platform_templates.py`、`platform_app/templates/platform_app/login.html`、相关测试、`CLAUDE.md`、本控制板与 Prompt OS 4.1 文档 | 前端 `App` 37 passed；`npm --prefix frontend run build` passed；后端导出/OSS/模板/N5-N6 聚焦 43 passed；`python manage.py check` passed；`git diff --check` passed（仅 Windows 换行提示） | 本地验证完成，部署备份预定 `/opt/independent-image-platform-backups/20260803_150636-r016-export-prompt-briefs` | 真实付费 1+8、50 张质量压测、ERP/OSS 浏览器 smoke 和完整 Prompt OS 历史测试收口待后续任务 |
| R0-17 | 主 Agent + Prompt OS + 前端体验 + 后端平台 | N5/N6 模型输出宽松传递与 fallback 生产门禁 | 用户最新确认：不要因 JSON 格式、字段缺失或空泛表达降级到系统 fallback；只有模型调用失败才失败或显式 fallback，DeepSeek 已返回的信息应继续传给后续节点 | N5/N6 先尝试结构化归一化，失败时保留非空 DeepSeek 文本为 `raw_model_text` 并标记 `prompt_source="deepseek"`；`PROMPT_OS_ALLOW_FALLBACK` 默认关闭，fallback 函数只允许测试/demo/无模型开发显式启用；fallback PromptVersion 在生产配置下不得进入正式生图；预备失败前端只显示“提示词生成失败，请重试预备生成” | `platform_app/services.py`、`image_platform/settings.py`、`docker-compose.yml`、`.env.example`、`frontend/src/components/ProductCard.tsx`、相关测试、`CLAUDE.md`、本控制板、运行手册与 Prompt OS 4.1 执行索引 | N5/N6/fallback 生图门禁聚焦 13 passed；前端 workbench 错误脱敏 18 passed；`python manage.py check` passed；`npm --prefix frontend run build` passed；`git diff --check` passed（仅 Windows 换行提示）；远端 Compose build/migrate/seed/check、`/health/ready`、前端 HTTP 200 和容器内 fallback 配置读回通过 | 已部署到预览；回退备份 `/opt/independent-image-platform-backups/20260803_170245-7fd286e-pre-r017-deepseek-fallback` | 完整 `tests/test_prompt_os.py` 仍有既有 N1/N2/旧 worker 断言失败；真实付费 1+8、50 张质量压测和 ERP/OSS 浏览器 smoke 待验收 |
| R0-18 | 主 Agent + Prompt OS + 前端体验 | 原始图 Prompt 槽隐藏与 N5/N6 宽松失败边界补强 | 用户指出原始商品图不该在商品卡里占提示词框；同时真实预备生成仍出现“提示词生成失败”，要求去掉 JSON/字段/Schema 这类限制，只在模型无返回或调用失败时失败 | `PromptEditor` 不渲染 `readOnly` 槽位，Shopee VN 01 原始图直通不再展示 Prompt 输入框；VN 专用槽位中文映射补齐；N5/N6 在 DeepSeek 已返回非空文本后，解析、Schema 或归一化异常都会按 `prompt_source="deepseek"` 原文继续，不走 deterministic fallback，也不创建生产 fallback PromptVersion | `platform_app/services.py`、`tests/test_prompt_os.py`、`frontend/src/components/PromptEditor.tsx`、`frontend/src/labels.ts`、`frontend/src/__tests__/workbench-v3.test.tsx`、本控制板 | N5/N6/fallback 聚焦 7 passed；前端 workbench 19 passed；`python manage.py check` passed；`npm --prefix frontend run build` passed；`git diff --check` passed（仅 Windows 换行提示）；远端 Compose build/migrate/seed/check、`/health/ready`、前端 HTTP 200、服务状态和 fallback 配置读回通过 | 已部署到预览（Git `d9d9c4c`）；回退备份 `/opt/independent-image-platform-backups/20260803_173246-d9d9c4c-pre-r018-source-prompt-fix` | 完整 `tests/test_prompt_os.py` 仍有既有 N1/N2/旧 worker 断言失败；真实付费 1+8 和 50 张质量压测待验收 |
| R0-19 | 主 Agent + Prompt OS + 前端体验 | 8 张 Prompt UI 与模型节点超时修复 | 用户指出隐藏原始图后提示词区仍写“1+8”且从 02 开始；新预备生成仍在 N5 失败，要求每个节点正常调用大模型，不要一直降级，并给出具体失败原因 | Prompt 编辑区按可见生成槽计数，原始图直通时标题为“8 张生成提示词”，白底 Prompt 显示为 01；商品详情标题与 N6 进度不再写“1+8”；APIMart Prompt 节点超时默认/Compose/示例配置调整为 120 秒；Chat Completions 支持列表式 content 文本提取；超时、限流、鉴权和空返回转成中文业务错误；N5/N6 模型失败不再 fallback 冒充正式 PromptVersion，生产关闭 fallback 时任何 fallback PromptVersion 均不得生图 | `platform_app/services.py`、`image_platform/settings.py`、`docker-compose.yml`、`.env.example`、`tests/test_apimart_client.py`、`tests/test_prompt_os.py`、`tests/test_generation_queue.py`、`frontend/src/components/ProductCard.tsx`、`frontend/src/components/PromptEditor.tsx`、`frontend/src/__tests__/workbench-v3.test.tsx`、本控制板 | APIMart/Prompt OS/生成门禁聚焦 28 passed；前端 workbench+prompt 30 passed；`python manage.py check` passed；`npm --prefix frontend run build` passed；`git diff --check` passed；服务器 20 秒 smoke 复现 N2/N3/N4/N7.tiktok/N8 timeout；服务器 120 秒临时 smoke 验证这些节点均返回；部署后生产容器 N1/N2/N3/N4/N5.generic/N5.shopee/N5.tiktok/N6.generic/N6.shopee/N6.tiktok/N7.generic/N7.shopee/N7.tiktok/N8/N9 均返回非空文本 | 已部署到预览（Git `7361e19`）；回退备份 `/opt/independent-image-platform-backups/20260803_181649-7361e19-pre-r019-model-timeout` | 完整 `tests/test_prompt_os.py` 仍有既有 N1/N2/旧 worker 断言失败；真实付费整套 1+8 质量和 50 张质量压测待运营验收 |
| R0-20 | 主 Agent + Prompt OS + 前端体验 + 后端平台 | 预备/出图真并发、提示词不空和编辑自动保存 | 用户指出提示词槽不能空、保存按钮多余、N7“检查”不是想要的体验；预备生成和正式出图都要并发，不应线程多却抢同一任务空跑；正式生成不应在预备中继续发请求 | Prompt Worker 商品领取改为数据库行锁/原子认领；Generation Worker 提交和轮询在第一个候选被抢后继续领取下一张，配合 `GENERATION_WORKER_CONCURRENCY` 真并发；前端有 `displayPrompt` 用中文稿，没有则显示实际模型 Prompt，不再清空成缺失；提示词输入失焦/短延迟自动保存，删除“保存提示词”按钮；预备中禁止正式生成但保留暂停；N7 员工文案改为“整理出图请求”，不再显示“检查/规则校验”；新建页只读当前用户角色，不加载完整工作台快照 | `platform_app/services.py`、`frontend/src/components/ProductCard.tsx`、`frontend/src/components/PromptEditor.tsx`、`frontend/src/pages/ProjectGrouping.tsx`、`frontend/src/layout.tsx`、`frontend/src/api.ts`、`frontend/src/types.ts`、`platform_app/views.py`、`platform_app/urls.py`、相关测试、运行手册、本控制板 | 前端 workbench+prompt 35 passed；Prompt Worker/current-user 后端 3 passed；Generation Worker 并发领取/归档聚焦 4 passed；`npm --prefix frontend run build` passed；`python manage.py check` passed | 本地验证完成，准备部署 | 完整 `tests/test_prompt_os.py` 仍有既有 N1/N2/旧 worker 断言失败；真实付费整套 1+8 质量和 50 张质量压测待运营验收 |
...
| P0-03 | 后端平台 | 商品准备、9 图生成、修订与选择式导出 | Prompt OS v2 与商品资料接口 | 分文件上传结果、结构化 PromptVersion、白底门禁、Shopee VN 原图直通、选择式导出、历史版本保留；导出门槛已由 R0-16 改为完成即默认可选 | Django 应用、迁移、测试、环境模板 | 后端 193 passed；迁移 0012、商品名/关系保存、PostgreSQL 行锁和准备重试 passed | 已部署 | 真实 ERP 员工账号成功登录/SKU 导入 smoke 待验收 |
...
| 2026-08-03 | N5/N6 非空 DeepSeek 输出优先保留，不因结构化失败 deterministic fallback | 用户确认过多限制会压制创意，JSON 格式并非业务失败；真正需要失败的是模型调用失败或空响应 | N5/N6 非空文本会作为 `prompt_source="deepseek"` 和 `raw_model_text` 进入下游；fallback 只在 `PROMPT_OS_ALLOW_FALLBACK=true` 时用于测试/demo/无模型开发，生产 fallback PromptVersion 不得进入正式生图 |
...
| 2026-08-03 | 生成完成后可直接进入选择式 ZIP，审核不再是导出门槛 | 运营要先批量拿到可用图，人工审核只做质量判断、修改和版本管理；旧的 accepted-only 门槛会导致 9/9 完成仍 0 张可导出 | 结果页默认勾选每个槽位最新完成图，员工可取消不满意版本；ZIP 后端按 completed + 有结果文件筛选；圈选修改和主动再生成仍创建新版本不覆盖历史 |
...
| 2026-08-03 | 导出按员工选中的完成版本、商品名称与 SKU 交付 | 员工需要默认全选又能排除不满意图，审核状态不能阻塞下载 | 默认最新完成版本；可选历史完成版本；ZIP 按“商品名称__SKU/01–09 槽位”组织 |
