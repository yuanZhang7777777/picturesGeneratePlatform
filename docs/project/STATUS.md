...
最后更新：2026-07-30
维护者：主 Agent
状态：Prompt OS、审核/导出后端、白底主图顺序门禁、ERP 登录切片与 OSS 对象存储已部署到 2 号服务器；OSS、ERP 登录和 SKU 查询 smoke 已通过，APIMart 真实 smoke 被服务器出站网络阻断，动态公平调度与 HTTPS 发布仍未启用
...
| P0-03 | 后端平台 | 审核、受限媒体、导出与 SKU 商品资料导入 API | Prompt OS 契约与商品资料接口响应 | 权限受控审核/导出/快照接口；ERP 登录影子用户、当前用户 Token 查询 SKU、私有 OSS 归档；同模板白底第 1 槽完成后才提交第 2–8 槽，失败/取消仅重做主图 | Django 应用、迁移、测试、Compose | API/权限/全量回归、服务器 health/OSS/ERP/SKU smoke | 进行中 | 本切片已部署到 2 号服务器；容器已拿到 ERP/OSS/APIMart 环境键，OSS 写读删通过。ERP 与 SKU 查询已纠正为 `103.198.125.2:16777` 并通过 smoke；不再使用 `103.198.125.5` |
...
| P0-06 | 主 Agent + 平台/QA | APIMart 真实契约与费用门禁 | APIMart 中文文档、公开测试图、精确目标模型 | 三模型的真实包络适配、结构化输出、异步任务恢复与质量基准证据 | 视觉/Prompt/图像适配器、测试、运行文档 | 公开图契约、标注基准集、失败语义、任务轮询与费用记录 | 进行中 | APIMart Base URL 已按官方 `https://api.apimart.ai/v1` 兼容并写入服务器 Secret；服务器到 `api.apimart.ai:443` 连接超时，真实 smoke 暂未通过。需修复 2 号服务器到 APIMart 的出站网络后再做付费契约 |
...
| 2026-07-30 | ERP 与 SKU 商品资料主机纠正为 `103.198.125.2` | 现有日更和服务器 smoke 均证明 `.2` 可达；业务确认不要使用 `103.198.125.5` | 服务器 `.env` 已切到 `103.198.125.2:16777`；`15672` 仅是早期 `.5` 示例/测试端口，不属于正式链路 |
...
| APIMart 从 2 号服务器出站不可达 | 真实付费生图无法在服务器上完成 smoke | 后端平台 + QA/发布 + 网络负责人 | 修复服务器出口 IP `139.224.2.166` 到 `api.apimart.ai:443` 的出站访问 |
