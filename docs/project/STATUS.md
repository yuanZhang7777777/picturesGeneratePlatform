...
最后更新：2026-08-02
维护者：主 Agent
状态：`lxc/workbench-v4` 已完成 Prompt OS 3.2 节点/工作流设计与 GPT-5.5 执行任务书，尚未实现或发布。当前生产继续使用 3.0.0；2026-08-02 本地基线为后端 353 项、前端 89 项通过，Django check 与 Vite build 通过。目标 3.2.0 将把工作台触发、预备/正式生成、员工错误文案、多图 `target_appearances`、Prompt 变量总线、五种营销策略和文案锁变成可验证契约；真实 ERP/OSS、新付费 1+8 和 50 商品浏览器验收仍未完成。
