...
最后更新：2026-08-01
维护者：主 Agent
状态：Git `241fdcc` 的 Workbench v4 已部署到 `hermes-remote` 技术预览。常驻导入、大商品卡、真实缩略图排序、固定侧浮层、多外观 1+8 和正式生成自动准备已通过后端 348 项、前端 84 项、Vite build、小样本浏览器验收、远端 Compose/健康与公网静态包读回；50 商品、真实 ERP/OSS 和新一轮付费 1+8 尚未完成，不能标记为业务验收完成。
