...
最后更新：2026-08-03
维护者：主 Agent
状态：`lxc/workbench-v4` 本轮 R0-19 本地验证完成，待部署；商品卡 Prompt 编辑区不再说“1+8”，原始图直通时只显示 8 张生成提示词并按 01–08 编号；生产 Prompt 节点超时从 20 秒提升到 120 秒，服务器实测 120 秒下 N2/N3/N4/N5/N6/N7/N8/N9 文本节点可返回；N5/N6 模型失败不再 deterministic fallback 冒充成功，失败时保存具体中文业务原因。Prompt OS 4.1 的真实付费 1+8、50 张质量压测、ERP/OSS 浏览器 smoke 和全量历史测试宽门禁收口仍未完成。
...
| R0-19 | 主 Agent + Prompt OS + 前端体验 | 8 张 Prompt UI 与模型节点超时修复 | 用户指出隐藏原始图后提示词区仍写“1+8”且从 02 开始；新预备生成仍在 N5 失败，要求每个节点正常调用大模型，不要一直降级，并给出具体失败原因 | Prompt 编辑区按可见生成槽计数，原始图直通时标题为“8 张生成提示词”，白底 Prompt 显示为 01；商品详情标题与 N6 进度不再写“1+8”；APIMart Prompt 节点超时默认/Compose/示例配置调整为 120 秒；Chat Completions 支持列表式 content 文本提取；超时、限流、鉴权和空返回转成中文业务错误；N5/N6 模型失败不再 fallback 冒充正式 PromptVersion，生产关闭 fallback 时任何 fallback PromptVersion 均不得生图 | `platform_app/services.py`、`image_platform/settings.py`、`docker-compose.yml`、`.env.example`、`tests/test_apimart_client.py`、`tests/test_prompt_os.py`、`tests/test_generation_queue.py`、`frontend/src/components/ProductCard.tsx`、`frontend/src/components/PromptEditor.tsx`、`frontend/src/__tests__/workbench-v3.test.tsx`、本控制板 | APIMart/Prompt OS/生成门禁聚焦 26 passed；前端 workbench 21 passed；服务器 20 秒 smoke 复现 N2/N3/N4/N7.tiktok/N8 timeout；服务器 120 秒临时 smoke 验证这些节点均返回，其中 N4 31.39 秒 | 本地验证完成，待部署 | 部署后需读回容器 `APIMART_PROMPT_TIMEOUT_SECONDS=120` 并重试刚才失败的 `test666` 两个商品；完整 `tests/test_prompt_os.py` 仍有既有 N1/N2/旧 worker 断言失败 |
