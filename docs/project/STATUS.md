...
最后更新：2026-07-31
维护者：主 Agent
状态：显式预备生成、紧凑商品工作台与 Prompt OS v3 重构进行中。当前分支正在实现“整理导入零 AI、预备生成 N1–N7、确认后正式生图”；尚未完成浏览器、服务器或真实付费验收。
...
| 2 ERP 与市场配置 | ERP/配置 + 前端市场选择 | 名称式新建项目、首次配置、商品继承/覆盖、两套导入 | 后端/前端回归、真实 ERP 登录、1 SKU、OSS、项目/商品配置 E2E | 实现中 | 待工作台重构完成后验证 |
| 3 Prompt OS 运行契约 | Prompt OS + 规则输入 | N1–N7 严格 Schema、当前证据、Generation 硬门禁和 Worker CAS | 节点契约、多市场 Prompt 回归、无门禁任务被拒绝 | 安全前置修复中 | 不代表阶段 2 已通过；待 API 对接和全量验证 |
...
| R0-01 | 主 Agent + 后端 + Prompt OS + 前端体验 | 显式预备生成、商品工作台与 Prompt OS v3 | 已确认 v3 实施计划、现有项目/商品/API、旧长 Prompt 原文 | 通用电商/东南亚通用默认值；整理导入零 AI；预备 N1–N7；整行展开卡片；generic/shopee/tiktok 营销链；当前证据硬门禁 | Django、`frontend/`、Prompt 模板与五份权威文档；不改 `用户操作流程以及相关触发.md` | 基线后端 297 passed、前端 74 passed；三 Agent 分文件实现中 | 进行中 | 集成、浏览器、部署、ERP/OSS 与真实付费 1+8 验收未完成 |
...
| 2026-07-31 | 工作台改为紧凑顶部、居中添加弹层和方形商品卡 | 上传/ERP 长期占位挤压商品整理空间，抽屉与列表不利于批量视觉确认 | 图片与 ERP 为同一弹层页签；卡片直接编辑名称、平台、国家、补充信息与单品风格；点击卡片在当前整行展开身份卡和九槽 Prompt |
...
| 项目 | 影响 | Owner | 处理条件 |
| --- | --- | --- | --- |
| R0-01 生成硬门禁仍在重构 | 在修复合并和全量验证前不能用当前分支做真实付费 1+8 质量结论 | 主 Agent + Prompt OS | 合格 PromptVersion/N7、最终引用验证、Worker CAS、全量测试与审查通过后才可做真实调用 |
