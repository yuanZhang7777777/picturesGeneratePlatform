...
最后更新：2026-07-31
维护者：主 Agent
状态：Git `e98ad9f` 为 workbench v4 实现基线。常驻导入、大商品卡、真实缩略图排序、固定侧浮层、多外观 1+8 和正式生成自动准备正在 `lxc/workbench-v4` 实现；上一版预览仍保持 `1ef34e1`，未通过本轮全量门禁前不部署。
...
| R0-02 | 执行 Agent（主 Agent 签核） | Workbench v4 与一卡多外观 1+8 | `e98ad9f`、R0-01 API/ClusterAsset/analysis_snapshot、2026-08-01 确认边界 | 常驻导入、大卡、真实缩略图排序/跨卡移动、固定侧浮层；`asset_order`；`target_appearances`/逐槽 `appearance_ids`；generate 自动准备续跑 | 六份权威文档、`CLAUDE.md`、现有 Django/React 与测试；禁止触碰未跟踪操作流程文档 | 文档契约已更新；代码/全量/浏览器/部署待执行 | 实现中 | 基线已有后端 3 失败、前端 10 失败，先更新为新行为测试再实现 |
