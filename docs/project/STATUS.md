...
最后更新：2026-07-30
维护者：主 Agent
状态：Prompt OS、审核/导出后端和白底主图顺序门禁已完成；ERP 登录、用户 Token 查询 SKU 与 OSS 对象存储进入实现切片，真实 APIMart、动态公平调度与 HTTPS 发布仍未启用
...
| P0-03 | 后端平台 | 审核、受限媒体、导出与 SKU 商品资料导入 API | Prompt OS 契约与商品资料接口响应 | 权限受控审核/导出/快照接口；ERP 登录影子用户、当前用户 Token 查询 SKU、私有 OSS 归档；同模板白底第 1 槽完成后才提交第 2–8 槽，失败/取消仅重做主图 | Django 应用、迁移、测试 | API/权限/全量回归 | 进行中 | 本切片已实现 ERP 登录、session Token 查询 SKU、OSS 存储适配与导出 ZIP 归档；仍需服务器 Secret 写入、真实 ERP/OSS smoke、动态公平并发和 HTTPS 发布 |
...
| 真实 APIMart、动态公平并发与 500 上限执行尚未启用 | 仍只能称为假模式技术预览，不能把供应商上限当成当前能力 | 后端平台 + QA/发布 | 服务器 Secret 配置、原子认领/限流、真实契约测试与分级压测 |
