...
最后更新：2026-08-03
维护者：主 Agent
状态：`lxc/workbench-v4` 本轮 R0-17 已完成本地验证，准备部署到预览；N5/N6 不再因 JSON 不合法、缺字段或普通空泛文本走 deterministic fallback，DeepSeek 非空输出会作为 `prompt_source="deepseek"` 原文传给下游；只有模型调用失败或空响应才预备失败或在显式开关下 fallback。Prompt OS 4.1 的真实付费 1+8、50 张质量压测、ERP/OSS 浏览器 smoke 和全量历史测试宽门禁收口仍未完成。
