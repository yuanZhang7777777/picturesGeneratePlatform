...
最后更新：2026-07-30
维护者：主 Agent
状态：Prompt OS、审核/导出后端、白底主图顺序门禁、ERP 登录切片与 OSS 对象存储已部署到 2 号服务器；OSS、ERP 登录和 SKU 查询 smoke 已通过，APIMart 三个基础节点已在本地开发机真实 smoke 通过，服务器侧仍被出站网络阻断，动态公平调度与 HTTPS 发布仍未启用
...
| P0-03 | 后端平台 | 审核、受限媒体、导出与 SKU 商品资料导入 API | Prompt OS 契约与商品资料接口响应 | 权限受控审核/导出/快照接口；ERP 登录影子用户、当前用户 Token 查询 SKU、私有 OSS 归档；同模板白底第 1 槽完成后才提交第 2–8 槽，失败/取消仅重做主图 | Django 应用、迁移、测试、Compose | API/权限/全量回归、服务器 health/OSS/ERP/SKU smoke | 进行中 | 本切片已部署到 2 号服务器；容器已拿到 ERP/OSS/APIMart 环境键，OSS 写读删通过。ERP 与 SKU 查询使用 `103.198.125.2:16777` 并通过 smoke |
...
| P0-06 | 主 Agent + 平台/QA | APIMart 真实契约与费用门禁 | APIMart 中文文档、公开测试图、精确目标模型 | 三模型的真实包络适配、结构化输出、异步任务恢复与质量基准证据 | 视觉/Prompt/图像适配器、测试、运行文档 | 公开图契约、标注基准集、失败语义、任务轮询与费用记录 | 进行中 | 本地开发机已完成 DeepSeek 文本、GPT-5 Nano 视觉观察、GPT Image 2 提交/轮询/结果下载最小真实 smoke；服务器到 `api.apimart.ai:443` 仍连接超时。需修复 2 号服务器出站网络后重跑同套 smoke |
...
| 2026-07-29 | 正式入口只要求账号登录，不使用来源 IP 白名单 | 内部员工从公网访问 | 仍必须使用域名 HTTPS 与账号级失败锁定；HTTP IP:端口只可做技术预览 |
...
| 2026-07-30 | APIMart 基础节点先在本地真实打通，服务器侧等 IT 出站修复后复测 | 本地网络可访问 APIMart，2 号服务器暂时超时 | `deepseek-v4-pro` 走非流式 Chat Completions；`gpt-5-nano-2025-08-07` 走 Responses，文本从 `output[].content[].text` 提取；`gpt-image-2` 先上传参考图，再用 URL 字符串数组 `image_urls` 提交生成，完成后下载结果归档 |
...
| APIMart 从 2 号服务器出站不可达 | 本地真实节点已通，但服务器真实付费生图无法完成 smoke | 后端平台 + QA/发布 + 网络负责人 | 修复服务器出口 IP `139.224.2.166` 到 `api.apimart.ai:443` 的出站访问 |
