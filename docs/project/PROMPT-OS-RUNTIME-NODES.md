...
- 第一张是主参考，N1 只识别这张主图，节省视觉模型费用和等待时间。
- 同卡其它图片不做前置视觉识别，N2 本地归并为最多 3 张支持参考图，正式生图时再交给 `gpt-image-2` 作为商品参考。
- 当前成本优先运行态不再让 N2/N3/N4 调 DeepSeek；N2 本地生成商品身份和参考图计划，N3 本地生成最小事实台账，N4 本地拼固定白底 Prompt。
- 因为副图不再前置识别，颜色/规格/款式的精细区分主要依赖主图、人工补充和正式生图参考图；员工需要强调某个款式时，应写进商品卡补充信息。
...
| 节点 | 模型 | 输入 | 输出 | 普通用户看到 |
|---|---|---|---|---|
| N1 主图观察 | `gpt-5-nano-2025-08-07` | 主参考图片、`asset_id`、人工/ERP 商品名、补充资料 | 是否有商品、候选名、可见结构、颜色、材质、图片角色、可用性 | 只看到“识别中/识别异常/请换图或补充名称” |
| N2 商品身份 | 本地规则 | N1 主图观察、商品名、补充资料、图片顺序 | 商品名、身份锁、主参考、最多 3 张辅助参考、`target_appearances` | 商品身份卡，可修改 |
| N3 事实台账 | 本地规则 | N1/N2、人工事实 | 最小 `observed` 事实、证据、用途边界 | 默认不展开，审核/高级编辑可看 |
| N4 白底 Prompt | 本地规则 | 商品身份、事实、平台规则、尺寸 | 固定标准白底产品图 Prompt | 第 1 张 Prompt |
| N5 套图策划 | `deepseek-v4-flash` | 商品身份、事实、平台/国家、项目/单品风格、8 个营销槽 | 每槽购买任务、场景、文案策略、`appearance_ids` | 中文槽位标题和策划 |
| N6 单图编译 | `deepseek-v4-flash` | 全部 N5 槽位计划、事实、语言、规则、参考图计划 | 全部槽位英文生图 Prompt、锁定的目标语言文案 | 每张图 Prompt，可编辑 |
| N7 规则检查 | 确定性规则，语义模型默认关闭 | 最终 Prompt、参考图、规则快照 | 通过/阻断/警告 | 只显示可操作中文错误 |
...
- N5 必须先发散再收敛：每个营销槽先从真实使用、问题解决、细节信任、尺度适配、通勤/旅行/工作/户外/礼物/收纳/亲子/宠物/社交等场景族中静默比较，再只输出最适合当前商品和购买问题的一种；不得八张图都退回同一种暖色家居、桌面摆拍或无购买理由的 flatlay。
- 对套装、组合、收纳盒、托盘、包装或配件，白底/总览/包含物图可以展示完整套装；使用、生活方式、情绪和细节图应让当前动作需要的功能部件成为主角。盒子、托盘、包装和额外配件只在帮助理解使用、携带、收纳或下单内容时作为辅助上下文。
