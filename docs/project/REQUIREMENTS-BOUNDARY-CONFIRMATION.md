...
- 商品名称、身份锁、整套要求和每张图的完整 Prompt 都允许员工修改。
- 平台规则不单独展示给员工；第一次生成 Prompt 时由系统自动带入。
- 员工可以修改或删除平台建议和身份约束；系统只保留以下不可绕过的技术边界：
  - 第 1 张必须是标准白底产品图。
...
最终：不设置正式审核步骤或审核员角色。图片技术生成成功后即可选择和导出。
你的答案：确认。

#### H02 是否允许批量通过

最终：结果页默认勾选全部最新成功图；员工只需取消不想导出的图片，或对不满意的单张发起修改/重做。多个项目可以并行生产。
你的答案：确认。
