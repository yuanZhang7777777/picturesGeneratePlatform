...
flowchart LR
  ...
  C --> D["N1/N2 识别商品"]
  D --> E["首次确认项目平台与国家"]
  E --> F["N3–N7 生成合格 Prompt"]
  F --> G["自动出图，或先拖拽/修改再生成"]
  G --> H["先生成白底图，再生成 8 张营销图"]
  H --> I["查看结果；默认全选"]
  I --> J["可选：圈选修改或重做单张"]
  I --> K["批量下载最新选中图到本地"]
```
...
- 新建项目只填写项目名。项目可先导入并完成 N1/N2 识别；首次保存项目平台和国家后，才允许 N3–N7、营销策划和任何付费生图。
- 项目顶部设置默认平台、国家、比例和分辨率；每个商品都可以单独覆盖自己的平台和国家。商品覆盖优先于项目默认，未覆盖商品始终跟随项目默认；以后修改项目默认不改写已有商品覆盖、历史 Prompt 或结果。
- 自动出图在首次配置前只记录生成意图；配置和准备都成功后再按商品排队，绝不以通用兜底 Prompt 付费生成。
...
- 不使用文件名作为临时商品名称；识别完成前名称字段保持空白，只显示“可不填，AI 将根据图片识别”占位提示。
