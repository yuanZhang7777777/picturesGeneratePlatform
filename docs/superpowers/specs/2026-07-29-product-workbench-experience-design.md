...
`deepseek-v4-pro` 是文本模型，不能直接阅读源图、竞品图或生成图；它只接收结构化 `VisualFactPacket`，再完成所有业务推理与 Prompt 编译。`gpt-5-nano-2025-08-07` 是业务已授权的唯一视觉观察器，目标模型也必须经过 APIMart 账户契约测试：精确模型 ID、图片输入、Schema 输出、超时/错误语义及内容安全错误均需通过；任何一项失败都阻断相应自动节点，不静默替换模型。该固定快照须在 2026-12-11 前完成替换模型的回归、管理员发布和回滚验证。
