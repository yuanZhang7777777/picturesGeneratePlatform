...
日期：2026-07-29
状态：历史体验稿；2026-07-30 起由 `2026-07-30-dual-speed-product-platform-design.md` 取代，本文只保留调研过程和可复用交互细节
...
1. ERP SKU 与图片/文件夹上传是两个并列入口，进入项目后共用同一商品确认板。SKU 查询仅取得 SKU、商品名和主图，不把销售、库存、成本或分类带入业务字段和 Prompt。
2. 默认“一张来源图 = 一个商品集群 = 一套 8 张图”。图片拖入同一集群即共同产出一套；集群可标记为同商品参考或多色/多款组合，默认仍保持一图一集群。
...
7. 图片技术完成不代表业务通过；人工审核保留用于质量判断、圈选修改和版本管理。导出以完成且有图为门槛，默认勾选最新完成版本；失败、驳回、Prompt、批注、圈选和历史版本永久保留。
...
| 新建项目 | 建立生产上下文 | 选择 ERP SKU 或图片/文件夹入口、选择市场/模板、填写种子风格 | 供应商参数与完整 Prompt |
| 商品确认板 | 确认“一套图对应什么商品” | 拖拽分配图片、拆分集群、选择图片关系、补商品资料 | 系统自动合并商品 |
...
- 新建项目显示“上传图片/文件夹”和“ERP SKU”两个同级入口；用户可选择任一方式进入统一商品确认板。
...
- 图片拖入商品卡后直接完成分配并可撤销；商品卡单独设置“同商品参考”或“多色/多款组合”。保持独立集群则分别拥有白底图、套图、审核与导出。
...
2. 默认一图一商品集群；拖拽可直接分配，员工能清楚区分“同商品参考”和“多色/多款组合”，每个集群只生成一套。
