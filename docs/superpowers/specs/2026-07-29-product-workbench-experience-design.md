...
- 专用视觉观察器目标模型为 APIMart 多模态 Responses 的 `gpt-4-vision`：只把我方源图、竞品图或生成候选图转换为严格 Schema 的 `VisualFactPacket`，不编写营销策略、生产 Prompt、合规结论或图片。
...
| 视觉观察 | 私有我方源图、竞品图或生成候选图与窄 Schema | `gpt-4-vision` 输出 `VisualFactPacket`：可见属性、未知项、置信度；竞品只输出抽象 Style DNA 候选，生成候选另输出身份偏差/风险 | Schema 解析失败仅重试一次；仍失败则阻断依赖该包的自动节点，不改由其他模型猜图 |
...
`deepseek-v4-pro` 是文本模型，不能直接阅读源图、竞品图或生成图；它只接收结构化 `VisualFactPacket`，再完成所有业务推理与 Prompt 编译。`gpt-4-vision` 是业务已授权的唯一视觉观察器，目标模型也必须经过 APIMart 账户契约测试：精确模型 ID、图片输入、Schema 输出、超时/错误语义及内容安全错误均需通过；任何一项失败都阻断相应自动节点，不静默替换模型。

视觉数据路径必须隔离：我方源图可传给 `gpt-4-vision` 观察器和 `gpt-image-2` 生成器；竞品原图仅传给 `gpt-4-vision` 观察器，保留为私有受权素材，绝不进入 DeepSeek、GPT Image 2、生产 Prompt、生成任务或导出包。观察器只保存经 Schema 校验的结果，不保存或展示模型自由推理；竞品结果只保留抽象 Style DNA，排除商标、包装文字、人物肖像、独特插画和可识别版式。
