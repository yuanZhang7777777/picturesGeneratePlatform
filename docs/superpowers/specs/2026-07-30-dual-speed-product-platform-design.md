...
系统默认自动完成图片识别、商品身份约束、营销策划、Prompt 和出图。员工只在合并多张素材、修正名称/Prompt、修改不满意图片和批量下载时介入。
...
flowchart LR
  ...
  E --> F["首次保存项目平台与国家"]
  F --> G{"本次导入方式"}
  G -->|自动出图| H["N3–N7 合格 Prompt"]
  G -->|导入后整理| I["拖拽/改名称/配置/Brief"]
  I --> H
  H --> J["先生成白底图"]
  J --> K["并行生成 8 张营销图"]
  K --> L["人工审核通过"]
...
- 新建项目只填写项目名。平台、国家、比例、分辨率和项目风格在进入项目后首次保存；未完成首次平台和国家配置前，可导入和完成 N1/N2 识别，但不能启动 N3–N7 或付费生成。

### 3.2 项目工作区

- 顶部压缩为一行：项目名称、项目默认配置、添加商品、批量选择、生成选中商品和生产与结果。默认配置使用紧凑弹层，店铺类型与模板仅放“更多设置”。
- “添加商品”使用统一抽屉，含“图片/文件夹”“ERP SKU”页签；唯一可见入口“添加图片或文件夹”展开原生“选择图片”（单张/多张）和“选择文件夹”。拖拽直接接受散图、多个图片或文件夹。
- 待导入区显示无文件名缩略图；成功文件立即移出并折叠抽屉，失败文件保留重试，后续导入不得再次提交已经成功的文件。
- 每张图片默认创建一个商品；ERP 自动填入 SKU、商品名和图片。商品名称允许为空，界面只显示“可不填，AI 将根据图片识别”占位提示。
- 图片拖入商品卡即合并并可撤销；关系只选“同商品参考”或“多色/多款组合”。
- 商品使用响应式方形卡片网格（每张最小约 230px，1920px 约 5–6 列，笔记本约 3–4 列）。上半部为主参考图，多图显示叠图和数量；不显示源文件名。
- 卡片下半部直接编辑商品名称、平台、国家/站点与补充信息（材质、功能、消费者、卖点或特别要求），并显示生成勾选和状态；全选、取消全选、反选以商品 ID 保存，轮询不得重置，0 个商品选中时禁止生成。
- 未手动覆盖的卡显示“跟随项目”；覆盖配置优先于项目默认，提供“恢复跟随项目”。项目默认变化只重新准备受影响商品，不改写历史 Prompt、Generation 或审核记录。
- 多图关系、删除/归档、身份锁和高级 Prompt 进入卡片更多菜单或右侧详情抽屉。
- 可删除单张参考图或整个商品。没有生成历史时物理删除业务记录和私有素材；存在历史时只归档隐藏；活跃生成或提交状态未知时拒绝删除。
- 商品卡提供“换一个 Prompt”；不增加 Brief 页、Prompt 审批页或预检页。
- `pending/preparing` 在卡内显示“AI 正在识别”；`blocked` 说明需补充内容或 ERP 名称与图片可能不一致。只有网络、认证失效或服务端 5xx 显示全局错误。生成请求逐商品返回排队、等待或阻断结果，等待中的自动意图在准备成功后继续排队。
...
| 节点 | 模型 | 输入 | 输出 |
| --- | --- | --- | --- |
| N1 逐图观察 | `gpt-5-nano-2025-08-07` | 每张我方商品图、名称和关系 | 严格 JSON 的可见商品证据；缺候选名、结构、图片角色或置信度仅修复一次 |
| N2 身份归并 | `deepseek-v4-pro` | N1 观察、ERP/人工名称、多图关系 | 严格 JSON 的主图、最多三张互补参考图、身份锁和图文冲突结论 |
...
九节点严格 JSON 契约、版本快照、失败分类和审核门禁以 [`节点prompt设定初稿.md`](节点prompt设定初稿.md) 为准。N1/N2 在未配置市场时可运行；N3–N7 必须在平台和国家已确认后运行。N2 批准的主图和参考图是唯一可进入后续生成数组的源素材；营销图优先使用已完成白底图，按需最多增加一张结构参考，避免复刻源图构图。N5 对决策任务、场景、环境、镜头、动作与构图做五维差异检查。合理推断可用于策划或待审核文案，但必须标记 `inferred`、置信度、风险和证据；价格、认证、疗效、减重、美容前后对比与站外导流等高风险内容不得仅靠推断进入图片。
...
- `Cluster` 增加多图关系、分析快照、准备状态、准备错误和是否自动生成；合并素材后重新进入准备队列。
- `Batch` 初始默认配置可为空；`Cluster.platform_override`、`market_override`、`seller_tier_override` 为可空覆盖。生效顺序为商品覆盖 → 项目默认 → 全球通用模板。
...
1. 新建项目只填项目名；可上传和完成 N1/N2，首次保存项目平台和国家后自动模式才可得到 1+8 共 9 张结果。
2. 上传多张图选择整理模式，可拖入同一商品卡并只生成一套。
