...
- 新建项目只填写项目名。进入工作台后默认显示“Shopee / 东南亚通用 / 1:1 / 1K”；平台、国家、比例、分辨率和项目风格均可在顶部直接修改。

### 3.2 项目工作区

- 顶部压缩为项目名称、平台、国家、比例、分辨率和项目风格；预备生成、正式生成和生产结果放在滚动常驻浮动操作条，员工在长列表任意位置都能操作选中商品。
- “添加商品”是页面常驻区域，图片/文件夹与 ERP SKU 同屏并排显示，不使用页签或抽屉；原生“选择图片”（单张/多张）和“选择文件夹”与拖入散图/文件夹共用现有导入链路。
