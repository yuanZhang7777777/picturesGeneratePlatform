# 双速 AI 商品出图平台设计

日期：2026-07-30
状态：业务已确认，作为实现设计基线
产品边界：`../../project/REQUIREMENTS-BOUNDARY-CONFIRMATION.md`

## 1. 目标

平台服务公司内部运营员工。员工可以上传图片/文件夹，也可以输入 ERP SKU；两种素材来源进入同一个商品工作区，最终为每个商品生成：

1. 1 张标准白底产品图。
2. 8 张电商营销图。

系统默认自动完成图片识别、商品身份约束、营销策划、Prompt 和出图。员工只在合并多张素材、修正名称/Prompt、修改不满意图片和批量下载时介入。

## 2. 最短业务流程

```mermaid
flowchart LR
  A["ERP 登录"] --> B["项目工作区"]
  B --> C["上传图片/文件夹"]
  B --> D["输入 ERP SKU"]
  C --> E{"本次导入方式"}
  D --> E
  E -->|自动出图| F["识别 + Prompt"]
  E -->|导入后整理| G["识别 + 商品卡"]
  G --> H["拖拽/改名称/市场/Prompt"]
  H --> I["生成"]
  F --> I
  I --> J["先生成白底图"]
  J --> K["并行生成 8 张营销图"]
  K --> L["结果默认全选"]
  L --> M["本地 ZIP"]
  L --> N["圈选修改或再生成一个版本"]
```

每次导入显示两个明确按钮：

- `导入并自动出图`：常用快速模式。识别和 Prompt 成功后直接生产。
- `导入后整理`：先进入商品卡工作区，再由员工生成。

系统记住该项目上次选择，但每次导入仍显示本次方式；不使用隐藏开关。低置信度商品名、无有效图片或模型不可用只暂停对应商品，不影响同项目其他商品。

## 3. 页面与导航

### 3.1 工作台

- 显示自己的项目、商品数、已完成数、失败数和正在生成数。
- 一个员工可同时打开多个项目；任务在服务端继续执行。
- 新建项目只填写项目名、平台/市场、比例、分辨率和可选种子风格。

### 3.2 项目工作区

- 顶部：项目默认平台、国家、比例、分辨率和项目风格。
- 主操作：上传素材、ERP SKU、生成选中商品。
- 每张图片默认创建一个商品卡；ERP 自动填入 SKU、商品名和图片。
- 图片拖入商品卡即合并并可撤销；关系只选“同商品参考”或“多色/多款组合”。
- 商品卡直接编辑名称、主图、市场覆盖、身份锁、整套要求和 9 个槽位 Prompt。
- 商品卡提供“换一个 Prompt”；不增加 Brief 页、Prompt 审批页或预检页。

### 3.3 生产与结果

- 按项目和商品展示 `识别中 / Prompt 生成中 / 排队 / 生成中 / 已完成 / 需处理`。
- 结果默认选中每个槽位的最新成功版本，成功即可以导出，不设置正式审核状态。
- 单张结果可查看历史、再生成一个版本、圈选修改或取消导出选择。
- 失败支持重做单张、当前商品失败项或项目全部失败项；成功项不会被批量失败重试重复提交。

### 3.4 管理后台

- 管理平台模板、市场规则来源、Prompt 节点版本、模型参数、队列和供应商错误。
- 初始管理员只有刘学城；普通员工只能查看自己的项目。
- 不做项目共享、外部通知、员工成本/额度页面或手机端适配。

## 4. 1+8 套图

| 顺序 | 作用 | 规则 |
| --- | --- | --- |
| 1 | 标准白底产品图 | 完整、真实、无促销文字/水印；任何平台都先生成 |
| 2 | 第二视角/结构 | 展示不同视角或结构 |
| 3 | 核心卖点 | 只使用已证实商品事实 |
| 4 | 材质/细节 | 材质不确定时不写精确声明 |
| 5 | 使用场景 | 按市场与消费者自动匹配 |
| 6 | 模特/使用对象 | 自动匹配婴儿、儿童、成人或宠物 |
| 7 | 尺寸/包装/包含物 | 未知规格不编造数字 |
| 8 | 平台转化营销图 | 按平台和市场语言规划 |
| 9 | 补充转化图 | 避免与前面槽位重复 |

槽位 2–9 的 Prompt 可以在白底图完成前编译，但不得提交 APIMart。白底图完成并归档后，其结果与我方原始参考图共同进入槽位 2–9 的商品参考数组。不同商品的白底图可并行。

## 5. AI 节点

| 节点 | 模型 | 输入 | 输出 |
| --- | --- | --- | --- |
| 商品观察 | `gpt-5-nano-2025-08-07` | 我方商品图 | 严格 JSON：名称候选、可见事实、身份锁、消费者、置信度 |
| 竞品观察 | `gpt-5-nano-2025-08-07` | 最多 4 张竞品图 | 严格 JSON：抽象 Style DNA |
| 营销策划与 Prompt | `deepseek-v4-pro` | 商品事实、市场、模板、种子风格、Style DNA | 套图策略和 9 个完整 Prompt |
| 图像生成/修订 | `gpt-image-2` | 当前槽位 Prompt 与我方参考图 | 单张结果或修订版本 |

竞品原图只进入竞品观察节点，绝不进入 DeepSeek、GPT Image 2、生成参考数组或导出包。

DeepSeek 使用非流式 Chat Completions。`temperature` 默认 `1.2`，配置范围 `0–2`；严格 JSON 解析失败时使用同一输入做一次修复重试，仍失败则只暂停该商品。该参数依据 [APIMart 通用对话接口](https://docs.apimart.ai/cn/api-reference/texts/general/chat-completions-nostream) 的公开范围，具体模型仍以服务器账户契约测试为准。

## 6. 数据与后台状态

- 保留现有 `Batch / Cluster / Asset / PromptVersion / Generation / ResultAsset`，不重建领域模型。
- `Cluster` 增加多图关系、分析快照、准备状态、准备错误和是否自动生成；合并素材后重新进入准备队列。
- `Batch` 只保存项目上次导入方式。每次导入请求仍显式携带 `auto_generate`。
- PromptVersion 和 Generation 快照继续不可覆盖；修改、失败重做和主动再生成都创建新版本。
- 旧 `review_status`、ReviewFeedback 和 ReviewAnnotation 数据保留以兼容历史，不再作为新 UI 或导出的门禁；圈选修改继续复用批注和新版本能力。
- 每日业务额度关闭；供应商活跃任务硬上限仍为 500。运行并发从 2 开始，按 2、5、8、50、100、250、500 压测提升。

## 7. 失败与暂停

- 员工暂停/取消只影响尚未提交供应商的任务；已提交任务继续轮询并归档。
- DeepSeek JSON 失败自动修复一次。
- 明确未受理的 429/5xx 按供应商 `Retry-After` 有限重试；未知是否受理的提交超时保持 `submit_unknown`，绝不自动重复 POST。
- 内容安全或无效 Prompt 不盲重试，提示员工换 Prompt 后重做。
- APIMart 不可用时仍可导入、分组和编辑；生成入口显示服务不可用，不切换其他模型。

## 8. 导出、存储与权限

- 默认导出每个槽位最新成功版本；员工可取消选择或手动选择历史成功版本。
- ZIP 为 `项目名称_日期/商品名称__主SKU/01–09_槽位名.原格式`，并包含 `导出清单.csv`。
- 保持供应商归档原图格式和最高可用规格，不转 JPG、不二次压缩。
- ZIP 临时生成并直接下载到员工本地，不写入 OSS；原图、ERP 图、Prompt、结果和历史版本写私有 OSS。
- ERP 密码只用于登录换取 Token；浏览器只保存 Django Session Cookie。真实账号登录入口即使只使用公网 IP + 端口，也必须启用员工设备信任的 HTTPS。

## 9. 技术边界

- React + TypeScript + Vite、TanStack Query、dnd-kit；Django 5.2、PostgreSQL、现有两类 Worker、Caddy 和 Docker Compose。
- 可见页面 3 秒轮询、后台 15 秒轮询、终态停止；不引入 WebSocket、Redis、消息队列、微服务或新的前端状态库。
- 不做独立审核中心、AI 质检、自由画布、完整在线 PS、多人实时协作、站外通知、移动端或长期 ZIP 存储。

## 10. 验收主线

1. 上传一张图选择自动模式，可无人干预得到 1+8 共 9 张结果。
2. 上传多张图选择整理模式，可拖入同一商品卡并只生成一套。
3. ERP 输入最多 50 个 SKU，只取 SKU、商品名和图片，后续流程与上传一致。
4. 白底图失败时同商品 8 张营销图不提交；重做白底成功后继续。
5. 成功图默认可导出；圈选修改和主动再生成产生新版本且旧版本仍可查看。
6. ZIP 只包含员工选中的成功版本，按商品名称/SKU 和 01–09 命名并下载到本地。
7. 普通员工不能读取他人项目；浏览器、日志、Prompt、导出包和 Git 不出现 Token 或 Secret。
