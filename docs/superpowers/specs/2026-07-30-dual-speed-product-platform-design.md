...
- 新建项目只填写项目名。进入工作台后默认显示“Shopee / 东南亚通用 / 1:1 / 1K”；平台、国家、比例、分辨率和项目风格提示词均可在顶部直接修改。

### 3.2 项目工作区

- 顶部压缩为一行：项目名称、平台、国家、比例、分辨率、项目风格提示词、批量选择和生产结果。国家使用中文下拉，优先展示东南亚通用、越南、泰国、菲律宾、马来西亚、新加坡、印度尼西亚；比例和分辨率分开选择。项目风格提示词使用可调大小的小文本框，并作为全项目种子提示词进入 8 张营销图 Prompt。
- “添加商品”是页面常驻区域，图片/文件夹与 ERP SKU 同屏并排显示，不使用页签或抽屉；原生“选择图片”（单张/多张）和“选择文件夹”按钮放在拖拽区域内，与拖入散图/文件夹共用现有导入链路。SKU 区提供明确“加载 SKU / 加载中 / 失败保留”状态。
- 待导入区显示无文件名缩略图；成功文件立即移出，失败文件保留重试，后续导入不得再次提交已经成功的文件。
- 每张图片默认创建一个商品；ERP 自动填入 SKU、商品名和图片。商品名称允许为空，界面只显示“可不填，AI 将根据图片识别”占位提示。
- 图片拖入商品卡即合并并可撤销；不再要求或显示多图关系。一个商品卡代表一套图，可以包含同商品多角度，也可以包含同商品族的不同颜色、规格或款式。拖整卡合并商品，拖单张缩略图可排序或跨卡移动，拖到空白区拆成新商品。排序后的第一张为主参考，其余为参考图，并使旧准备结果失效。
- 商品使用响应式紧凑大卡片网格，桌面宽屏优先一行约 5 个商品。上半部以 `object-contain` 完整显示当前预览图；点击任意缩略图只切换大图预览，不改变主参考。下方横向显示全部真实缩略图，仅在溢出时滚动；只有拖动缩略图排序到第一位才改变主参考；不使用叠图占位，不显示源文件名。
...
- 状态按 `待整理 → N1 → N2 → N3 → N4 → N5 → N6 x/8 → N7 → 预备完成 → 白底生成 → 营销图 x/8 → 待审核 → 已完成` 展示。点击预备或正式生成后，选中商品立即进入脉冲进度状态，随后由轮询替换为真实节点进度。只有网络、认证失效或服务端 5xx 显示全局错误；单商品等待或失败不影响其他商品。模型结构化技术错误不得原样暴露给员工。
...
| N2 身份归并 | `deepseek-v4-pro` | 全部 N1 观察、ERP/人工名称、当前商品素材集合 | 商品家族共有身份锁、`target_appearances`、主参考、参考图和图文冲突结论；同外观多角度归并，不同颜色/规格/款式区分为目标外观；仅全部图无有效商品时阻断 |
...
N2 的目标不是把所有图片硬压成同一个外观，而是输出两层变量：商品家族共有身份锁与目标外观集合。白底图、款式总览图必须覆盖全部目标外观；其他营销槽可按购买决策选择子集，但整套 1+8 必须覆盖全部目标外观。

Prompt 质量框架参考公开生产建议：OpenAI 的 GPT Image 2 指南强调清晰结构、显式约束和逐步迭代；Adobe Firefly 产品图教程把参考图、姿态/动作、构图、光线和质感拆开控制；Shopify 的媒体生成说明把商品背景、场景、灯光和自然语言 Brief 作为核心输入。平台只借鉴“结构化创意 Brief”方法，不复制第三方商品、品牌、人物、原文案或具体版式。
...
7. 成功图完成后即可默认进入选择式导出；人工审核保留用于质量判断、圈选修改和版本管理，但不再作为 ZIP 门槛。
8. ZIP 只包含员工选中的完成版本，按商品名称/SKU 和 01–09 命名并下载到本地。
