...
- 新建项目只填写项目名。进入工作台后默认显示“通用电商 / 东南亚通用 / 1:1 / 1K”；平台、市场、比例、分辨率和项目风格均可在顶部直接修改。

### 3.2 项目工作区

- 顶部压缩为一行：项目名称、平台、市场、比例、分辨率、项目风格、添加商品、预备生成、正式生成和生产结果。平台使用“通用电商 / Shopee / TikTok Shop”直接按钮，常用市场使用中文按钮，更多国家进入可搜索面板。
- “添加商品”使用居中弹层，含“图片/文件夹”“ERP SKU”页签；唯一可见入口“添加图片或文件夹”展开原生“选择图片”（单张/多张）和“选择文件夹”。拖拽直接接受散图、多个图片或文件夹。
- 待导入区显示无文件名缩略图；成功文件立即移出并关闭或折叠弹层，失败文件保留重试，后续导入不得再次提交已经成功的文件。
- 每张图片默认创建一个商品；ERP 自动填入 SKU、商品名和图片。商品名称允许为空，界面只显示“可不填，AI 将根据图片识别”占位提示。
- 图片拖入商品卡即合并并可撤销；不再要求员工选择多图关系。拖整卡合并商品，拖单张缩略图移动素材，拖到空白区拆成新商品。
...
- 点击商品卡后，该卡占满当前整行：原方形卡保留在左侧，右侧展示身份卡、事实台账和 1+8 Prompt。同一时间只展开一个；已展开时第一次点击外部只关闭并消费该操作，第二次点击另一商品才展开。内部编辑不关闭，拖拽仍直接执行。
...
| N2 身份归并 | `deepseek-v4-pro` | N1 观察、ERP/人工名称、当前商品素材集合 | 严格 JSON 的主图、最多三张互补参考图、身份锁和图文冲突结论 |
...
- `Batch` 初始默认配置为通用电商、东南亚通用、1:1、1K；`Cluster.platform_override`、`market_override` 为可空覆盖。生效顺序为商品覆盖 → 项目默认 → 通用电商模板。
...
1. 新建项目只填项目名并获得通用电商/东南亚通用默认配置；自动模式按当前配置完成 N1–N7 后得到 1+8，整理模式在点击预备生成前没有 AI 调用。
