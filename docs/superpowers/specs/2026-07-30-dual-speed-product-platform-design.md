...
| 节点 | 模型 | 输入 | 输出 |
| --- | --- | --- | --- |
| N1 逐图观察 | `gpt-5-nano-2025-08-07` | 每张我方商品图 | 可见商品证据 JSON |
| N2–N6 | `deepseek-v4-pro` | 观察、资料、市场、模板、种子风格 | 身份归并、推断台账、白底编译、营销策划和单槽 Prompt |
| N7 规则闸门 | 后端确定性规则引擎 | 最终 Prompt、事实、规则与参考图 | 付费提交阻断/通过 |
...
九节点严格 JSON 契约、版本快照、失败分类和审核门禁以 [`节点prompt设定初稿.md`](节点prompt设定初稿.md) 为准。合理推断可用于策划或待审核文案，但必须标记 `inferred`、置信度、风险和证据；价格、认证、疗效、减重、美容前后对比与站外导流等高风险内容不得仅靠推断进入图片。
...
4. 白底图失败时后续营销图不提交；默认模板重做成功后继续 8 张，Shopee VN 普通店继续槽位 3–9 的 7 张。
5. 成功图必须人工审核通过才可导出；圈选修改和主动再生成产生新版本且旧版本仍可查看。
6. ZIP 只包含员工选中的审核通过版本，按商品名称/SKU 和 01–09 命名并下载到本地。
