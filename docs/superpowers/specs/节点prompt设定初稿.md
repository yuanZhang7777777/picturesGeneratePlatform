...
> 2026-08-02 已补充 Prompt OS 4.0 节点生产设计。当前生产仍以运行发布版本为准；3.1–3.4 只保留为历史推演，4.0 作为下一轮 GPT-5.5 执行基线，收口节点输入输出、商品多图外观、五种营销文案策略、目标语言文字锁、员工错误脱敏、前端进度和正式生成门禁。见 [Prompt OS 4.0 节点生产设计](./2026-08-02-prompt-os-4.0-node-production-design.md)。实施完成并发布前，不得把 4.0.0 写成现有运行能力。
