...
> 2026-08-02 已补充 Prompt OS 3.4 节点与执行设计。当前生产仍以运行发布版本为准；3.1 只覆盖营销文案契约，3.2 覆盖工作台与多图链路，3.3 合并两者，3.4 进一步收口多图外观、触发状态、员工错误脱敏、目标语言文字锁和 GPT-5.5 执行任务书。见 [Prompt OS 3.4 节点与执行设计](./2026-08-02-prompt-os-3.4-node-execution-design.md)。实施完成并发布前，不得把 3.4.0 写成现有运行能力。
