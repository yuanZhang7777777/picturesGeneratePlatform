...
> 2026-08-02 Prompt OS 3.1 Task 7 已新增本地非付费六类营销基准：家居厨卫、宠物、婴幼儿、穿戴、美妆、工具/电器各 8 个营销槽，共 48 槽。基准使用现有 N5 归一化与 N7 文案锁检查，要求每套至少四种策略、至少一张 FAB、拟人至多一张、重复场景签名 0、无效事实引用 0、文字锁错误 0、空泛模板文案 0，并要求至少 42 槽的相关性、具体性、自然度、可信度不低于 85。本地基准不调用 APIMart，不代表真实付费 1+8 质量验收已完成。
...
给 `gpt-image-2` 的最终 `prompt` 控制指令必须是英文。消费者可见文字先由 N6 生成并自检为目标市场语言，要求流畅、无歧义、符合当前场景和商品事实；这些 `localized_copy.lines` 是冻结文本，最终 Prompt 只能逐字引用，不允许图像模型再翻译、改写、增删或自由生成额外文字。N7 必须再次检查本地化文字质量，机器直译、语义歧义、场景不合或可能误导消费者时进入 `semantic_risks/warnings`，不阻断出图；只有价格/折扣、认证/奖项、医疗/疗效、100%/绝对承诺、站外导流、未授权 IP、危险/色情/暴力/仇恨/儿童安全等高风险内容才 `block`。
...
5. 不得取消、降低或改写真正的技术/高风险 hard_blocks；平台建议、文案质量和商品识别不完整不能升级成 hard block。
6. 不确定时输出 semantic_risks/warnings 并要求人工复核，不虚构官方规则。
...
- 确定性规则引擎异常：只在 Prompt 过期、无可用我方参考、竞品图误入参考、Prompt 超长或高风险文案时阻断；其他检查异常带警告继续。
- DeepSeek 语义补充失败：保留确定性结论并带警告通过；不得因为语义补充失败而阻断普通商品出图。
...
7. N7 只硬阻断 Prompt 超长、竞品参考、过期证据、无可用参考图和高风险可见内容；语言、文案质量、平台建议和普通规则风险进入人工复核提示，不阻断出图。
