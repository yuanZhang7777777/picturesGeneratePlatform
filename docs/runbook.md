...
2026-07-30 核查：现有 Hermes 日更 ERP 自动化使用旧 ERP 主机 `103.198.125.2:16777/8077`，2 号服务器当前可连通；本出图平台按业务新给的正式商品资料接口使用 `103.198.125.5:16777`，而 2 号服务器到 `103.198.125.5:16777/8077` 超时，只有旧测试端口 `103.198.125.5:15672` 可连通。因此“老日更能跑”不能证明新商品资料接口已放行。
...
- 目的 2：ERP/销售系统全部 API 主机，至少包含 `103.198.125.2`、`103.198.125.5` 与 `121.46.237.218`，TCP `16777`、`8077`、`15672`、`8071`，方向为出站；其中 `103.198.125.5:16777` 是当前出图平台配置的 ERP open login 与 SKU 商品资料查询入口，`103.198.125.2:16777/8077` 是现有日更自动化可达的旧 ERP 入口，`121.46.237.218:8071` 是当前销售系统 API。
...
timeout 8 bash -lc '</dev/tcp/103.198.125.5/15672'
