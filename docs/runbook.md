...
2026-07-30 纠正：ERP 登录与 SKU 商品资料查询使用 `103.198.125.2:16777`，不要使用 `103.198.125.5`。`15672` 只来自早期 `.5` 示例/测试端口，不属于当前正式链路，也不应放进 IT 申请或平台配置。
...
- 目的 2：ERP/销售系统 API 主机包含 `103.198.125.2` 与 `121.46.237.218`，方向为出站；其中 `103.198.125.2:16777` 是当前出图平台配置的 ERP open login 与 SKU 商品资料查询入口，`103.198.125.2:8077` 是现有日更自动化可达的 ERP 业务入口，`121.46.237.218:8071` 是当前销售系统 API。
