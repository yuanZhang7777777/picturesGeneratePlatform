...
6. 人物、婴幼儿、成人或宠物只在与商品真实用途相符且平台规则允许时出现，动作必须表现正确接触、佩戴、握持、安装或支撑关系。
7. 内部结构、包装包含物、精确尺寸、配件和性能只有存在证据时才能安排。
8. seed_style 和 Style DNA 只影响抽象视觉策略，不得复刻竞品品牌、原文字、人物、插画或可识别版式。
9. coverage_check 必须明确报告重复、空槽和高风险推断。
...
5. 人物、婴幼儿、成人或宠物必须符合商品目标消费者并正确使用商品，不能只是站在旁边；证据不足时采用中性静态展示。
...
3. 不得把 ADVICE 升级成平台硬规则，除非另有明确 HARD_PLATFORM 或 HARD_MALL 规则引用。
4. 不得因为文案营销性强就自动判违规；必须指出具体规则和具体内容。
5. 不得取消、降低、删除或改写确定性引擎已有 hard_blocks。
6. 不确定时加入 semantic_risks 并要求人工复核，不虚构官方规则。
7. pass 只表示允许提交付费生成，不代表图片审核通过或允许导出。
