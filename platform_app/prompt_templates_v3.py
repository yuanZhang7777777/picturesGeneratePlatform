...
4. 套装、组合、收纳盒、托盘、包装或配件要按槽位处理：白底、总览、包含物和下单确认槽位可展示完整套装；真实使用、生活方式、情绪触发和细节槽位应优先展示当前动作需要的功能部件。若商品包含多个共同完成用途的核心件，使用类槽位要规划自然共用子集，而不是只抓最显眼或最容易生成的单件。收纳盒、托盘、包装和额外配件只作为辅助上下文，不能因为源图包含它们就在套图中反复成为主角。
