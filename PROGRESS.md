...
# Workbench v4 import/action and Prompt preparation hotfix

- Status: deployed 2026-08-01 as Git `66ab314` to `hermes-remote:/opt/independent-image-platform` under `global.lock`.
- Scope: N1 now normalizes common owned product roles such as `product/product_detail/packaging`; N2 placeholder `string` identity output falls back to N1 observed product name/category/appearance instead of polluting the product card. The workbench now shows image/folder and ERP SKU import in one compact permanent panel, keeps “导入后整理” as the primary action, removes the visible multi-image relation selector, uses larger object-contain product cards, changes UI copy to “国家”, and provides a bottom-right persistent pre-generate/formal-generate action bar for long product lists.
- Verification: backend full pytest passed; frontend 84 passed; Vite build passed; Django check passed; `git diff --check` passed with only Windows CRLF warnings. `makemigrations --check --dry-run` timed out and was not retried because no model or migration files changed.
- Deployment: remote Compose build/up, Django check, migrations, template seed, five service status, local `/health/ready`, public `/health/ready`, and deployed commit readback passed. Backup: `/opt/independent-image-platform-backups/20260801_233655-66ab314`; `global.lock` released.
