...
状态：阶段 1“批量整理工作台可用化”已部署到 2 号云服务器 `18083` 预览入口；代码、自动测试、生产构建、浏览器文件夹导入、迁移和五服务健康门禁均通过。当前入口仍是测试预览，不是面向 100 名员工的正式 HTTPS 发布。唯一执行顺序见[分阶段交付路线图](docs/superpowers/plans/2026-07-31-phased-delivery-roadmap.md)。
...
- 当前线上运行代码为 Git `d5410bb`；迁移 `0013`、五服务、健康检查、无文件名上传预览、浏览器原生文件夹导入、紧凑商品列表、选择、删除/归档和历史保护已通过。真实 ERP 员工登录与 SKU 导入仍需人工 E2E。
