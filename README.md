...
状态：Workbench v4 与 Prompt OS v3 已部署到 2 号云服务器 `18083` 预览入口；常驻导入、大商品卡、横向真实缩略图、固定侧浮层和一卡多外观 1+8 已通过自动回归与浏览器验收。真实员工 ERP 浏览器导入、50 商品压力场景和新一轮付费 1+8 质量验收仍待执行。当前入口仍是测试预览，不是面向 100 名员工的正式 HTTPS 发布。唯一执行顺序见[分阶段交付路线图](docs/superpowers/plans/2026-07-31-phased-delivery-roadmap.md)。

本项目面向公司内部运营人员，提供“上传图片/文件夹”和“ERP SKU”两个并列入口。每次导入可选择“导入并自动出图”或“导入后整理”：整理模式只保存素材和商品卡，员工主动点击“预备生成”才执行主图观察、本地身份/事实/白底编译、营销策划和规则闸门；自动模式复用相同 N1–N7 链路并在通过后继续生产。默认套图为 1 张白底图 + 8 张营销图；Shopee VN 普通店为真实原图 + 白底图 + 7 张营销图。Django 负责登录、权限、任务和数据；`frontend/` 中的 React + TypeScript + Vite 工作台由 Caddy 同源提供静态文件，并由 Caddy 代理 Django API、认证、后台、健康检查和旧 `/batches/` 链接重定向。
...
- 最多约 100 名内部员工。
- 目标每日约 2,000 张生成图提交；供应商和服务器并发需按运行手册分级压测后逐步提高。
- 供应商 API 活跃异步任务上限为 500；当前预览默认图片活跃生成并发为 32，必须完成限流与分级压测后才可逐步提高。
- 2 号云服务器独立 Docker Compose 栈。
- 临时预览入口使用当前空闲的 `18083` 端口；正式使用不做来源 IP 白名单，按业务决定使用公网 IP + 端口，但真实 ERP 账号登录前必须启用员工设备信任的 HTTPS。

当前预览边界：

- 部署目录：`/opt/independent-image-platform`。
- Compose 项目名：`independent-image-platform`。
- 服务器已配置真实 ERP、OSS 与 APIMart Secret；OSS 写读删 smoke 已通过。迁移后的 2 号服务器已完成 APIMart 三节点最小真实 smoke，并完成一个真实 1+8 项目：DeepSeek/GPT-5 Nano/GPT Image 2 全链路、9 张结果 OSS 归档、人工审核通过和 ZIP 导出。ERP 登录接口网络可达；真实员工账号成功登录/SKU 导入 smoke 需通过浏览器人工登录或服务器端专用 smoke 账号变量完成。
...
- `APIMART_FAKE_MODE=1` 仍是本地和新环境的唯一安全默认值；当前 2 号服务器预览环境已由主 Agent 在受限 smoke 中切到真实 APIMart。提高并发或开放正式素材前仍必须完成限流验收和主 Agent 发布签核。
- HTTP 的 IP:端口入口只允许测试账号与非敏感素材；在公网 IP + 端口的 HTTPS 和账号安全就绪前，不得面向 100 名员工开放。
- 当前线上运行代码为 Git `b7ef9b7`；Prompt OS v4.1 成本优先运行态已上线，N1 只识别主参考图，N2/N3/N4 本地生成，N5/N6 使用 DeepSeek。五服务、Django check、健康检查和公网入口读回均通过；真实员工浏览器 SKU/OSS、50 商品场景和新一轮付费 1+8 仍需人工验收。
...
- 当前成本优先运行态下，N1 只识别主参考图；N2/N3/N4 本地生成，不调用 DeepSeek；DeepSeek 只用于 N5 商品级营销策划和 N6 商品级全槽位图片设计。`3500` 字符限制只作用于最终单图生成/修改 Prompt。
...
- 生成成功且有结果文件的最新版本默认进入选择式 ZIP，不再以人工审核作为导出门槛。员工可以取消选择、选择历史完成版本、圈选修改或主动再生成。失败重做和人工修改均保留旧版本。

## Worker 边界

- `generation-worker` 提交、轮询并归档图片任务。
- `prompt-worker` 执行商品视觉理解、结构化 Prompt 生成和 9 槽 PromptVersion 准备。
