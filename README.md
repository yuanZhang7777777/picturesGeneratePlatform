...
状态：显式“预备生成”、紧凑商品工作台与 Prompt OS v3 已部署到 2 号云服务器 `18083` 预览入口；真实员工 ERP 浏览器导入和新一轮付费 1+8 质量验收仍待运营执行。当前入口仍是测试预览，不是面向 100 名员工的正式 HTTPS 发布。唯一执行顺序见[分阶段交付路线图](docs/superpowers/plans/2026-07-31-phased-delivery-roadmap.md)。
...
- 当前线上运行代码为 Git `1ef34e1`；迁移 `0015`、Prompt OS v3 模板发布、五服务和健康检查已通过。新工作台提供页面常驻导入区、整理导入零 AI、显式预备 N1–N7、正式生成硬门禁、放大商品卡、拖拽合并/移动/拆分、预备进度条和中文市场配置；真实员工浏览器登录、SKU 预览与 OSS 归档仍需人工 E2E。
