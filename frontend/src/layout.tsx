...
export function ErrorPanel({ error, retry }: { error: unknown; retry?: () => void }) { const apiError = error instanceof ApiError ? error : undefined; const loginRequired = apiError?.authRequired || apiError?.status === 401; const title = loginRequired ? "登录已失效或需修改密码" : apiError?.status === 403 ? "访问被拒绝" : "无法读取服务器数据"; return <section className="surface max-w-xl p-6" role="alert"><h2 className="font-semibold text-rose-700">{title}</h2><p className="mt-2 text-sm text-slate-600">{apiError?.message ?? "请检查网络或稍后重试。"}</p><div className="mt-5 flex gap-2">{retry && <button className="secondary-button" onClick={retry}>重试</button>}{loginRequired && <a className="primary-button" href="/login/">前往登录</a>}</div></section>; }
...
export function statusText(status: string) { return ({ draft: "待配置", pending: "待准备", queued: "排队中", running: "生成中", processing: "生成中", completed: "已完成", ready: "可生成", failed: "需处理" } as Record<string, string>)[status] ?? status; }
