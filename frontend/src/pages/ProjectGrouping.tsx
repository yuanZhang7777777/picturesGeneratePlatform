...
export default function ProjectGrouping() {
  ...
  return <Shell>
    ...
    {importOpen && <div className="mb-5"><ImportPanel disabled={upload.isPending || skuImport.isPending || generate.isPending} onUpload={(files, mode) => upload.mutateAsync({ files, mode })} onSkuImport={(skus, mode) => skuImport.mutateAsync({ skus, mode })} onImported={() => setImportOpen(false)} /></div>}
    ...
    {globalError && <div className="mb-5"><ErrorPanel error={globalError} retry={() => void projectQuery.refetch()} /></div>}
