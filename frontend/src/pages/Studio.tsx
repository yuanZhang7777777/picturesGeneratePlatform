...
import { ApiError, confirmProject, updateCluster } from "../api";
...
export default function Studio() {
  ...
  const selected = sku.outputs.find((output) => output.id === selectedGenerationId) ?? latest[0];
  const assets = project.assets.filter((asset) => sku.assetIds.includes(asset.id));
  return <Shell><PageHeading eyebrow={`${project.name} / ${sku.name}`} title="商品创作台" action={<Link className="secondary-button" to={`/projects/${project.id}`}>返回商品分组</Link>} /><div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)_330px]"><aside className="surface p-4"><p className="section-label">商品参考图</p><div className="mt-3 grid grid-cols-2 gap-3">{assets.map((asset) => asset.imageUrl ? <img key={asset.id} className="aspect-square w-full rounded-xl object-cover" src={asset.imageUrl} alt={asset.name} /> : <div key={asset.id} className="aspect-square rounded-xl bg-slate-100" />)}</div><label className="mt-6 block text-sm font-medium text-slate-700"><span className="mb-2 block">身份锁</span><textarea value={identityLock} onChange={(event) => setIdentityLock(event.target.value)} /></label></aside><section className="surface p-5"><div className="flex items-center justify-between"><div><p className="section-label">当前输出</p><h2 className="mt-1 text-xl font-semibold">{selected?.name ?? "准备生成"}</h2></div><span className={`status status-${selected?.status ?? "draft"}`}>{statusText(selected?.status ?? "draft")}</span></div><div className="mt-5 grid min-h-96 place-items-center overflow-hidden rounded-2xl bg-slate-100">{selected?.imageUrl ? <img className="h-full max-h-[520px] w-full object-contain" src={selected.imageUrl} alt={selected.name} /> : <p className="text-sm text-slate-400">确认生成后将在这里显示结果</p>}</div><div className="mt-5 flex gap-2 overflow-x-auto">{sku.outputs.map((output) => <button key={output.id} aria-label={`${output.slot} v${output.attempt}`} className={`slot-button ${selected?.id === output.id ? "slot-button-active" : ""}`} onClick={() => setSelectedGenerationId(output.id)}>{output.slot}<span>v{output.attempt}</span></button>)}</div></section><aside className="surface p-5"><p className="section-label">AI Brief</p><label className="mt-4 block text-sm font-medium text-slate-700"><span className="mb-2 block">商品卖点与规格</span><textarea value={facts} onChange={(event) => setFacts(event.target.value)} /></label><label className="mt-4 block text-sm font-medium text-slate-700"><span className="mb-2 block">画面说明</span><textarea value={brief} onChange={(event) => setBrief(event.target.value)} /></label>{persist.isError && <ErrorPanel error={persist.error} retry={() => persist.mutate()} />}{generate.isError && <ErrorPanel error={generate.error} retry={() => generate.mutate()} />}<div className="mt-5 flex gap-2"><button className="secondary-button" disabled={persist.isPending} onClick={() => persist.mutate()}>保存 Brief</button><button className="primary-button" disabled={generate.isPending} onClick={() => generate.mutate()}>确认批量生成</button></div></aside></div></Shell>;
}
