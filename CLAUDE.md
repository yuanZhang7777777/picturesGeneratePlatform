...
- Prompt OS 4.1 是下一轮 GPT-5.5 唯一执行基线；3.1–4.0 只保留为历史推演。4.1 的关键边界是：导入整理零 AI、正式生成缺 Prompt 自动预备、一卡多图共同产出一套 1+8、N2 输出 `product_family` 与 `target_appearances`、N5 按 FAB/场景占有/情绪触发/拟人表达/身份表达动态策划，并输出“商品事实→用户结果→使用画面→情绪触发→文案角度”的 `copywriting_chain`；N6 先生成三条目标语言文案候选并自评，再把选中文案逐字锁进英文生图 Prompt；N7 无通过快照不得付费生图；节点失败先内部归一化或单商品阻断，不能把 `image_role`、Schema、JSON、`string`、英文模型错误暴露给员工。
