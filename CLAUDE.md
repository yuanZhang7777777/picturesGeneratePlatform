...
- 每张上传图片默认形成一个商品分组；图片拖入同一分组即共同产出一套图，不让员工维护“多图关系”。分组内第一张图片是主参考，员工可拖动横向缩略图排序；成本优先运行态下 N1 只识别主参考图，同卡其它图片不做前置视觉识别，只作为后续 `gpt-image-2` 商品参考图保留。
...
- Prompt OS 4.1 是下一轮 GPT-5.5 唯一执行基线；3.1–4.0 只保留为历史推演。4.1 的关键边界是：导入整理零 AI、正式生成缺 Prompt 自动预备、一卡多图共同产出一套 1+8、N2 输出 `product_family` 与 `target_appearances`、N5 按 FAB/场景占有/情绪触发/拟人表达/身份表达动态策划，并输出“商品事实→用户结果→使用画面→情绪触发→文案角度”的 `copywriting_chain`；N6 先生成三条目标语言文案候选并自评，再把选中文案逐字锁进英文生图 Prompt；N5/N6 非空 DeepSeek 文本即使不是合法 JSON，也要以 `prompt_source="deepseek"` 和 `raw_model_text` 传给下游，只有模型调用失败或空响应才失败或在显式 `PROMPT_OS_ALLOW_FALLBACK=true` 时 fallback；fallback PromptVersion 在生产配置下不得进入正式生图；N7 采用宽门禁，平台建议、文案质量、普通语言风险和身份不完整只进人工复核提示，只有 Prompt 超长、版本过期、无可用我方参考、竞品图误入参考和高风险可见内容才硬阻断；节点失败先内部归一化，不能把 `image_role`、Schema、JSON、`string`、英文模型错误暴露给员工。
- 当前成本优先运行态：N2 商品身份归并、N3 事实台账和 N4 白底 Prompt 均由本地规则生成，不调用 DeepSeek；DeepSeek 只用于 N5 商品级营销导演和 N6 商品级全槽位图片设计。N4 白底 Prompt 直接使用商品名、主图识别信息和同卡参考图拼固定白底指令。
